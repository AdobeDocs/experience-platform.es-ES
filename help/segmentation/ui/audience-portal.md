...
Para abrir Audience Portal, seleccione la ficha **[!UICONTROL Browse]** en la sección Segmentación.
...
![Se muestra la lista de acciones rápidas para las audiencias con el origen de [!UICONTROL Audience composition].](../images/ui/audience-portal/browse-audience-composition-details.png)

| Acción | Orígenes | Descripción |
| ------ | ------- | ----------- |
| [!UICONTROL Edit] | Servicio de segmentación | Abre el Generador de segmentos para editar la audiencia. Tenga en cuenta que si la audiencia se creó mediante la API, **no** podrá editarla con el Generador de segmentos. Para obtener más información sobre el uso del Generador de segmentos, lea la [guía de la interfaz de usuario del Generador de segmentos](./segment-builder.md). |
| [!UICONTROL Open composition] | Composición de público | Abre la composición Audiencia para ver su audiencia. Para obtener más información sobre la composición de audiencias, lea la [guía de interfaz de usuario sobre composición de audiencias](./audience-composition.md). |
| [!UICONTROL Activate to destination] | Servicio de segmentación | Activa la audiencia en un destino. Para obtener información más detallada sobre cómo activar una audiencia en un destino, lea [información general de activación](../../destinations/ui/activation-overview.md). |
| [!UICONTROL Share with partners] | Composición de audiencias, carga personalizada, servicio de segmentación | Comparte la audiencia con otros usuarios de Experience Platform. Para obtener más información sobre esta característica, lea [Resumen de coincidencia de segmentos](./segment-match/overview.md). |
| [!UICONTROL Manage tags] | Composición de audiencias, carga personalizada, servicio de segmentación | Administra las etiquetas definidas por el usuario que pertenecen a la audiencia. Para obtener más información sobre esta característica, lea la sección sobre [filtrado y etiquetado](#manage-audiences). |
| [!UICONTROL Move to folder] | Composición de audiencias, carga personalizada, servicio de segmentación | Administra a qué carpeta pertenece la audiencia. Para obtener más información sobre esta característica, lea la sección sobre [filtrado y etiquetado](#manage-audiences). |
| [!UICONTROL Copy] | Servicio de segmentación | Duplica la audiencia seleccionada. Encontrará más información sobre esta función en [Preguntas frecuentes sobre la segmentación](../faq.md#copy). |
| [!UICONTROL Apply access labels] | Composición de audiencias, carga personalizada, servicio de segmentación | Administra las etiquetas de acceso que pertenecen a la audiencia. Para obtener más información sobre las etiquetas de acceso, lea la documentación sobre [administración de etiquetas](../../access-control/abac/ui/labels.md). |
...
| [!UICONTROL Deactivate] | Carga personalizada, servicio de segmentación | Desactiva la audiencia seleccionada. Tenga en cuenta que para desactivar una audiencia, **no se puede** activar en **ningún** destino (incluidos destinos que no sean de Experience Platform) ni formar parte de **ningún** otro tipo de audiencias. Para obtener más información sobre la administración del estado del ciclo vital, lea la [sección de estado del ciclo vital de las Preguntas frecuentes sobre la segmentación](../faq.md#lifecycle-states). |
| [!UICONTROL Delete] | Composición de audiencias, carga personalizada, servicio de segmentación | Elimina la audiencia seleccionada. Las audiencias que se usan en destinos de flujo descendente o que dependen de otras audiencias **no se pueden** eliminar. Para obtener más información sobre la eliminación de audiencias, lea las [preguntas frecuentes sobre la segmentación](../faq.md#lifecycle-states). |
| [!UICONTROL Add to package] | Composición de audiencias, carga personalizada, servicio de segmentación | Mueve la audiencia entre zonas protegidas. Para obtener más información sobre esta característica, lea la [guía de herramientas para zonas protegidas](../../sandboxes/ui/sandbox-tooling.md). |
...
Si se cambia **[!UICONTROL Schedule all audiences]**, se habilitará la segmentación programada. Encontrará más información sobre la segmentación programada en la sección [segmentación programada de esta guía del usuario](#scheduled-segmentation).

Si selecciona **[!UICONTROL Import audience]**, podrá importar una audiencia generada externamente. Para obtener más información sobre cómo importar audiencias, lea la sección sobre [importación de una audiencia en la guía del usuario](#import-audience).

Si selecciona **[!UICONTROL Create audience]**, podrá crear una audiencia. Para obtener más información sobre cómo crear audiencias, lea la sección sobre [creación de una audiencia en la guía del usuario](#create-audience).
...
Puede seleccionar **[!UICONTROL Evaluation summary]** para mostrar un gráfico circular que muestre un resumen de las evaluaciones de audiencia.
...
| [!UICONTROL Update frequency] | Un valor que indica la frecuencia con la que se actualizan los datos de la audiencia. Los valores posibles de este campo incluyen [!UICONTROL Batch], [!UICONTROL Streaming], [!UICONTROL Edge] y [!UICONTROL Not Scheduled]. |
| [!UICONTROL Last updated by] | El nombre de la persona que actualizó la audiencia por última vez. |
| [!UICONTROL Created] | La fecha y la hora, en UTC, en que se creó la audiencia. |
| [!UICONTROL Last updated] | La fecha y la hora, en UTC, en que se actualizó la audiencia por última vez. |
| [!UICONTROL Tags] | Las etiquetas definidas por el usuario que pertenecen a la audiencia. Encontrará más información sobre estas etiquetas en la sección [de etiquetas](#tags). |
| [!UICONTROL Description] | La descripción de la audiencia. |
| [!UICONTROL Created by] | El nombre de la persona que creó la audiencia. |
| [!UICONTROL Access labels] | Las etiquetas de acceso para la audiencia. Las etiquetas de acceso le permiten categorizar conjuntos de datos y campos según las políticas de uso que se aplican a esos datos. Estas etiquetas se pueden aplicar en cualquier momento, lo que proporciona flexibilidad en la forma en que se decide administrar los datos. Para obtener más información sobre las etiquetas de acceso, lea la documentación sobre [administración de etiquetas](../../access-control/abac/ui/labels.md). |
| [!UICONTROL Breakdown] | El desglose del estado del perfil de la audiencia. A continuación, se puede encontrar una descripción más detallada de este desglose del estado del perfil. |

Si se selecciona desglose, la pantalla muestra un gráfico de barras que describe el porcentaje de perfiles que pertenecen a cada uno de los siguientes estados de perfiles calculados: [!UICONTROL Realized], [!UICONTROL Existing] y [!UICONTROL Exiting]. Además, el desglose que se muestra en la ficha [!UICONTROL Browse] es el desglose más preciso del estado de la definición del segmento. Si este número difiere de lo indicado en la ficha [!UICONTROL Overview], debe usar los números de la ficha [!UICONTROL Browse] como fuente de información correcta, ya que los números de la ficha [!UICONTROL Overview] solo se actualizan una vez al día.

| Estado | Descripción |
| ------ | ----------- |
| [!UICONTROL Realized] | Recuento de perfiles que **calificaron** para la audiencia en las últimas 24 horas desde que se ejecutó el último trabajo de segmentación por lotes. |
| [!UICONTROL Existing] | Recuento de perfiles que **permanecieron** en la audiencia en las últimas 24 horas desde que se ejecutó el último trabajo de segmentación por lotes. Este campo es **calculado** y no aparece en el objeto [`segmentMembership`](../../xdm/field-groups/profile/segmentation.md). |
| [!UICONTROL Exiting] | Recuento de perfiles que **abandonaron** la audiencia en las últimas 24 horas desde que se ejecutó el último trabajo de segmentación por lotes. |

Después de seleccionar los campos que desea mostrar, también puede cambiar el tamaño del ancho de las columnas mostradas. Puede hacerlo arrastrando el área entre las columnas o seleccionando el ![icono de flecha](/help/images/icons/chevron-down.png) de la columna cuyo tamaño desea cambiar, seguido de **[!UICONTROL Resize column]**.
...
Para agregar una etiqueta, seleccione **[!UICONTROL Manage tags]** en la audiencia que quiera etiquetar.

![El botón [!UICONTROL Manage tags] está seleccionado para una audiencia especificada.](../images/ui/audience-portal/browse-manage-tags.png)

Aparece la ventana emergente **[!UICONTROL Manage tags]**. En esta ventana emergente, puede seleccionar una etiqueta clasificada o una etiqueta sin clasificar.

| Tipo de etiqueta | Descripción |
| -------- | ----------- |
| Categorizado | Una etiqueta que crean y administran los administradores de su organización. |
| Sin categoría | Una etiqueta que se crea en la ventana emergente [!UICONTROL Manage tags]. Cualquiera puede crear o administrar este tipo de etiquetas. |

![Se muestra la ventana emergente [!UICONTROL Manage tags]. Se resaltan las opciones para elegir una categoría o una categoría sin categoría.](../images/ui/audience-portal/create-tag.png)

Después de agregar todas las etiquetas que desea adjuntar a la audiencia, seleccione **[!UICONTROL Save]**.

![En la ventana emergente [!UICONTROL Manage tags], las etiquetas agregadas están resaltadas.](../images/ui/audience-portal/created-tags.png)
...
Para crear una carpeta para albergar sus audiencias, seleccione **[!UICONTROL Create folder]**.

![El botón Crear carpeta está resaltado.](../images/ui/audience-portal/create-folder.png)

>[!NOTE]
>
>Solo puede crear una carpeta si está dentro de otra carpeta. Esto significa que **no puede** crear una carpeta si tiene **[!UICONTROL All Audiences]** seleccionado en la barra de navegación izquierda.

Aparece una ventana emergente que le permite asignar un nombre a la carpeta recién creada. Seleccione **[!UICONTROL Save]** después de darle un nombre a la carpeta para terminar de crearla. Tenga en cuenta que los nombres **deben** ser exclusivos de la carpeta principal.

![El botón Guardar del cuadro de diálogo Crear carpeta está resaltado.](../images/ui/audience-portal/create-folder-dialog.png)

Para mover una audiencia a una carpeta, seleccione **[!UICONTROL Move to folder]** de la audiencia que desee mover.

![El botón [!UICONTROL Move to folder] está seleccionado para una audiencia específica.](../images/ui/audience-portal/browse-move-to-folder.png)

Aparece la ventana emergente **Mover audiencia a la carpeta**. Seleccione la carpeta a la que desee mover la audiencia y, a continuación, seleccione **[!UICONTROL Save]**.
...
| [!UICONTROL Has any tag] | Le permite filtrar por etiquetas. Puede seleccionar entre **[!UICONTROL Has any tag]** y **[!UICONTROL Has all tags]**. Cuando se selecciona **[!UICONTROL Has any tag]**, las audiencias filtradas incluirán **cualquiera** de las etiquetas que haya agregado. Cuando se selecciona **[!UICONTROL Has all tags]**, las audiencias filtradas deben incluir **todas** las etiquetas que haya agregado. |
| [!UICONTROL Lifecycle status] | Le permite filtrar según el estado del ciclo vital de la audiencia. Las opciones disponibles son [!UICONTROL Deleted], [!UICONTROL Draft], [!UICONTROL Inactive] y [!UICONTROL Published]. |
| [!UICONTROL Update frequency] | Le permite filtrar según la frecuencia de actualización de la audiencia (método de evaluación). Las opciones disponibles incluyen [!UICONTROL Batch], [!UICONTROL Streaming] y [!UICONTROL Edge] |
| [!UICONTROL Created by] | Le permite filtrar según la persona que creó la audiencia. |
| [!UICONTROL Creation date] | Le permite filtrar según la fecha de creación de la audiencia. Puede elegir un intervalo de fechas para filtrar cuándo se creó la audiencia. |
| [!UICONTROL Modified date] | Le permite filtrar según la última fecha de modificación de la audiencia. Puede elegir un intervalo de fechas para filtrar cuándo se modificó la audiencia por última vez. |
...
Para ver más detalles acerca de una audiencia específica, seleccione el nombre de una audiencia en la ficha **[!UICONTROL Browse]**.
...
La sección **[!UICONTROL Audience summary]** proporciona información como el ID, nombre, descripción, origen y detalles de los atributos.
...
Seleccionar **[!UICONTROL Activate to destination]** le permite activar la audiencia en un destino. Para obtener información más detallada sobre cómo activar una audiencia en un destino, lea [información general de activación](../../destinations/ui/activation-overview.md).

![El botón Activar en destino está resaltado.](../images/ui/audience-portal/audience-details-activate.png)

Seleccionar **[!UICONTROL Apply access labels]** le permite administrar las etiquetas de acceso que pertenecen a la audiencia. Para obtener más información sobre las etiquetas de acceso, lea la documentación sobre [administración de etiquetas](../../access-control/abac/ui/labels.md).
...
![Se muestra la página de detalles de audiencia con el botón [!UICONTROL Open composition] resaltado.](../images/ui/audience-portal/audience-details-open-composition.png)

Si selecciona **[!UICONTROL Open composition]**, podrá ver la audiencia en la composición de la audiencia. Para obtener más información sobre la composición de audiencias, lea la [guía de la interfaz de usuario de la composición de audiencias](./audience-composition.md).

>[!TAB Carga personalizada]

![Se muestra la página de detalles de audiencia con el botón [!UICONTROL Update audience] resaltado.](../images/ui/audience-portal/audience-details-update-audience.png)

Si selecciona **[!UICONTROL Update audience]**, puede volver a cargar una audiencia generada externamente. Para obtener más información sobre cómo importar una audiencia generada externamente, lea la sección sobre [importación de una audiencia](#import-audience).

Si la audiencia está a siete días de caducar, puede seleccionar **[!UICONTROL Refresh data expiration]** para volver a ingerir el último lote correcto para la audiencia.

![El botón [!UICONTROL Refresh data expiration] está resaltado en la página de detalles de la audiencia.](../images/ui/audience-portal/refresh-data-expiration.png)

Esto amplía la caducidad de los datos en el número de días definido en la carga inicial y utiliza los mismos metadatos de audiencia. Si la caducidad de los datos no se ha definido anteriormente, se amplía en 30 días. Una vez que se ha presionado el botón, no se puede seleccionar durante otros 20 minutos.
...
![Se muestra la página de detalles de audiencia con el botón [!UICONTROL Edit audience] resaltado.](../images/ui/audience-portal/audience-details-edit-audience.png)

Seleccionar **[!UICONTROL Edit audience]** le permite editar su audiencia en el Generador de segmentos. Para obtener información más detallada sobre el uso del área de trabajo [!DNL Segment Builder], lea la [[!DNL Segment Builder] guía del usuario](./segment-builder.md).

>[!ENDTABS]

Si selecciona **[!UICONTROL Edit properties]**, podrá editar los detalles básicos de la audiencia, como el nombre, la descripción y las etiquetas.
...
Para audiencias y composiciones generadas por Experience Platform, la sección **[!UICONTROL Audience total]** muestra el número total de perfiles aptos para la audiencia.
...
Para audiencias con un origen de **[!UICONTROL Custom upload]**, la sección **[!UICONTROL Ingestion details]** muestra tanto el total del perfil como los detalles del conjunto de datos en el que se ingirió la audiencia generada externamente.
...
La sección **[!UICONTROL Activated destinations]** muestra los destinos para los que está activada esta audiencia.
...
Puede seleccionar **[!UICONTROL Create audience]** para crear una audiencia.
...
## Importación de un público {#import-audience}

>[!CONTEXTUALHELP]
>id="platform_segmentation_importaudience_dataexpiration"
>title="Caducidad de datos"
>abstract="El número de días que caducará la membresía del público. Este valor puede establecerse entre 1 y 90 días. Una vez que caduque el abono del público, la evaluación se detendrá y todos los perfiles abandonarán dicho abono del público. Puede actualizar el período de caducidad cuando queden menos de 7 días para la membresía del público. Sin embargo, una vez que quedan 0 días, no podrá actualizar el período de caducidad."

>[!IMPORTANT]
>
>Para importar una audiencia generada externamente, usted **debe** tener los siguientes permisos: [!UICONTROL View segments], [!UICONTROL Manage segments] y [!UICONTROL Import audience]. Para obtener más información sobre estos permisos, lea la [descripción general del control de acceso](../../access-control/home.md#permissions).

Puede seleccionar **[!UICONTROL Import audience]** para importar una audiencia generada externamente.
...
Aparece el flujo de trabajo **[!UICONTROL Import audience CSV]**. Puede seleccionar un archivo CSV para importarlo como una audiencia generada externamente.

![En el flujo de trabajo [!UICONTROL Import audience CSV], se resalta el cuadro [!UICONTROL Drag and drop files], que muestra dónde puede cargar la audiencia generada externamente.](../images/ui/audience-portal/import-audience-csv.png)
...
Después de seleccionar el archivo CSV para importar, se muestra una lista de datos de ejemplo para esta audiencia generada externamente. Después de confirmar que los datos de ejemplo son correctos, seleccione **[!UICONTROL Next]**.
...
Aparecerá la página **[!UICONTROL Audience details]**. Puede agregar información sobre la audiencia, incluido su nombre, descripción, identidad principal y valor de área de nombres de identidad.
...
![Se muestra la página [!UICONTROL Audience details].](../images/ui/audience-portal/import-audience-audience-details.png)
...
>[!NOTE]
>
>La caducidad de los datos personalizados establece el número de días que los perfiles permanecerán **1&rbrace; en la pertenencia a audiencias.** Una vez finalizada la caducidad de los datos, los perfiles abandonan la pertenencia a la audiencia. Este valor puede establecerse de 1 a 90 días.

Después de rellenar los detalles de la audiencia, seleccione **[!UICONTROL Next]**.

![El botón [!UICONTROL Next] está resaltado en la página [!UICONTROL Audience details].](../images/ui/audience-portal/import-audience-filled-details.png)

Se muestra la página **[!UICONTROL Review]**. Puede revisar los detalles de la audiencia recién importada y generada externamente.

![Se muestra la página [!UICONTROL Review] con los detalles de la audiencia recién importada generada externamente.](../images/ui/audience-portal/import-audience-review-details.png)

Después de confirmar que los detalles son correctos, seleccione **[!UICONTROL Finish]** para importar la audiencia generada externamente en Adobe Experience Platform.
