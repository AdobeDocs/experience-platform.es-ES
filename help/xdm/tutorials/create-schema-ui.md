...
## Abrir el área de trabajo [!UICONTROL Schemas] {#browse}

El área de trabajo [!UICONTROL Schemas] en la interfaz de usuario [!DNL Experience Platform] proporciona una visualización de [!DNL Schema Library], lo que le permite ver y administrar los esquemas disponibles para su organización. El área de trabajo también incluye [!DNL Schema Editor], el lienzo en el que puede crear un esquema a lo largo de este tutorial.

Después de iniciar sesión en [!DNL Experience Platform], seleccione **[!UICONTROL Schemas]** en el panel de navegación izquierdo para abrir el área de trabajo **[!UICONTROL Schemas]**. La ficha **[!UICONTROL Browse]** muestra una lista de esquemas (una representación de [!DNL Schema Library]) que puede ver y personalizar. La lista incluye el nombre, el tipo, la clase y el comportamiento (registro o serie temporal) en los que se basa el esquema, así como la fecha y la hora de la última modificación.
...
Para comenzar a componer un esquema, seleccione **[!UICONTROL Create schema]** en la esquina superior derecha del área de trabajo **[!UICONTROL Schemas]**.

![La ficha [!UICONTROL Schemas] del área de trabajo [!UICONTROL Browse] con [!UICONTROL Create schema] resaltado.](../images/tutorials/create-schema/create-schema-button.png)
...
Aparece el flujo de trabajo [!UICONTROL Create schema]. A continuación, elija una clase base para el esquema. Puede elegir entre las clases principales de [!UICONTROL XDM Individual Profile] y [!UICONTROL XDM ExperienceEvent], o [!UICONTROL Other] si estas clases no se ajustan a sus propósitos. La opción [!UICONTROL Other] clases permite [crear una nueva clase](#create-new-class) o elegir entre otras clases preexistentes.

Consulte la documentación de [[!UICONTROL XDM individual profile]](../classes/individual-profile.md) y [[!UICONTROL XDM ExperienceEvent]](../classes/experienceevent.md) para obtener más información sobre estas clases. Para los propósitos de este tutorial, seleccione **[!UICONTROL XDM Individual Profile]** seguido de **[!UICONTROL Next]**.

![Flujo de trabajo [!UICONTROL Create schema] con las opciones [!UICONTROL XDM individual profile] y [!UICONTROL Next] resaltadas.](../images/tutorials/create-schema/individual-profile-base-class.png)

### Nombre y revisión {#name-and-review}

Después de seleccionar una clase, aparece la sección [!UICONTROL Name and review]. En esta sección, proporcione un nombre y una descripción para identificar el esquema. Hay varias consideraciones importantes que se deben tener en cuenta al decidir un nombre para el esquema:
...
>[!NOTE]
>
>Puede actualizar el nombre para mostrar y la descripción opcional del esquema en la barra lateral **[!UICONTROL Schema properties]**. Una vez introducido un nuevo nombre, el lienzo se actualiza automáticamente para reflejar el nuevo nombre del esquema.
...
![Cuadro de diálogo [!UICONTROL Add field groups].](../images/tutorials/create-schema/field-group-popularity.png)
...
![Se resaltó el cuadro de diálogo [!UICONTROL Add field groups] con los grupos de campos del sector.](../images/tutorials/create-schema/industry-field-groups.png)
...
![Se resaltó el cuadro de diálogo [!UICONTROL Add field groups] con el icono de vista previa del grupo de campos seleccionado.](../images/tutorials/create-schema/preview-field-group-button.png)

Al obtener una vista previa de un grupo de campos, se proporciona una descripción detallada del esquema del grupo de campos en el carril derecho. También puede navegar por los campos del grupo de campos en el lienzo proporcionado. A medida que selecciona diferentes campos, el carril derecho se actualiza para mostrar detalles sobre el campo en cuestión. Seleccione **[!UICONTROL Back]** cuando haya terminado de obtener la vista previa para volver al cuadro de diálogo de selección de grupos de campos.

![Vista previa del cuadro de diálogo [!UICONTROL Preview field group] con el grupo de campos Detalles demográficos.](../images/tutorials/create-schema/preview-field-group.png)

Para este tutorial, seleccione el grupo de campos **[!UICONTROL Demographic Details]** y luego seleccione **[!UICONTROL Add field group]**.

![Se seleccionó el cuadro de diálogo [!UICONTROL Add field groups] con el grupo de campos Detalles demográficos y se resaltó [!UICONTROL Add field groups].](../images/tutorials/create-schema/demographic-details.png)
...
Este grupo de campos aporta varios campos bajo el nombre de nivel superior `person` con el tipo de datos &quot;[!UICONTROL Person]&quot;. Este grupo de campos describe información sobre un individuo, incluido el nombre, la fecha de nacimiento y el sexo.
...
Observe que el campo `name` tiene un tipo de datos de &quot;[!UICONTROL Full name]&quot;, lo que significa que también describe un concepto común y contiene subcampos relacionados con el nombre, como nombre, apellidos, título de cortesía y sufijo.
...
Ahora puede repetir los mismos pasos para agregar otro grupo de campos. Cuando vea el cuadro de diálogo **[!UICONTROL Add field group]** esta vez, observe que el grupo de campos &quot;[!UICONTROL Demographic Details]&quot; ha quedado atenuado y que la casilla de verificación que hay junto a él no se puede seleccionar. Esto evita que duplique accidentalmente grupos de campos que ya ha incluido en el esquema actual.

Para este tutorial, seleccione los grupos de campos estándar **[!UICONTROL Personal Contact Details]** y **[!UICONTROL Loyalty Details]** de la lista y, a continuación, seleccione **[!UICONTROL Add field groups]** para agregarlos al esquema.

![Cuadro de diálogo [!UICONTROL Add field groups] con dos nuevos grupos de campos seleccionados y [!UICONTROL Add field groups] resaltados.](../images/tutorials/create-schema/more-field-groups.png)

El lienzo vuelve a aparecer con los grupos de campos agregados enumerados en **[!UICONTROL Field groups]** en la sección **[!UICONTROL Composition]** y sus campos compuestos agregados a la estructura de esquema.
...
El esquema [!UICONTROL Loyalty Members] está diseñado para capturar datos relacionados con los miembros de un programa de fidelidad, y el grupo de campos estándar [!UICONTROL Loyalty Details] que agregó al esquema proporciona la mayoría de ellos, incluido el tipo de programa, los puntos, la fecha de unión, etc.
...
1. Ampliar el grupo de campos estándar [!UICONTROL Loyalty Details] con campos personalizados. Esto hace que [!UICONTROL Loyalty Details] se convierta en un grupo de campos personalizados y que el grupo de campos estándar original ya no esté disponible. Consulte la guía de la interfaz de usuario [!UICONTROL Schemas] para obtener más información sobre [agregar campos personalizados a la estructura de grupos de campos estándar](../ui/resources/schemas.md#custom-fields-for-standard-groups).

Para crear un nuevo grupo de campos, seleccione **[!UICONTROL Add]** en la subsección **[!UICONTROL Field groups]** como antes, pero esta vez seleccione **[!UICONTROL Create New Field group]** cerca de la parte superior del cuadro de diálogo que aparece. A continuación, se le pedirá que proporcione un nombre para mostrar y una descripción para el nuevo grupo de campos. Para este tutorial, asigne un nombre al nuevo grupo de campos &quot;[!DNL Custom Loyalty Details]&quot; y seleccione **[!UICONTROL Add field groups]**.

![Se resaltó el cuadro de diálogo [!UICONTROL Add field groups] con [!UICONTROL Create new field group], [!UICONTROL Display name] y [!UICONTROL Description].](../images/tutorials/create-schema/create-new-field-group.png)
...
&quot;[!DNL Custom Loyalty Details]&quot; debería aparecer ahora bajo **[!UICONTROL Field groups]** en el lado izquierdo del lienzo, pero aún no hay campos asociados a él y, por lo tanto, no aparecen nuevos campos bajo **[!UICONTROL Structure]**.
...
Aparece un marcador de posición &quot;[!UICONTROL Untitled Field]&quot; en el lienzo y el carril derecho se actualiza para mostrar las opciones de configuración del campo.

![Se resaltó el editor de esquemas con un [!UICONTROL Untitled Field] y el esquema [!UICONTROL Field properties].](../images/tutorials/create-schema/untitled-field.png)

En esta situación, el esquema debe tener un campo de tipo de objeto que describa en detalle el nivel de lealtad actual de la persona. Usando los controles del carril derecho, empiece a crear un campo `loyaltyTier` con el tipo &quot;[!UICONTROL Object]&quot; que se utilizará para contener los campos relacionados.

En **[!UICONTROL Assign to]**, debe seleccionar un grupo de campos al que asignar el campo. Recuerde que todos los campos de esquema pertenecen a una clase o a un grupo de campos y, como este esquema utiliza una clase estándar, la única opción es seleccionar un grupo de campos. Empiece a escribir el nombre &quot;[!DNL Custom Loyalty Details]&quot; y, a continuación, seleccione el grupo de campos de la lista.

Cuando termine, seleccione **[!UICONTROL Apply]**.

![Se ha resaltado el editor de esquemas con el objeto de nivel de fidelidad agregado al esquema [!UICONTROL Field properties].](../images/tutorials/create-schema/loyalty-tier-object.png)
...
Seleccione el icono **más (+)** junto al objeto `loyaltyTier` para empezar a agregar subcampos. Aparece un nuevo marcador de posición de campo y la sección **[!UICONTROL Field properties]** está visible en el lado derecho del lienzo.
...
* **[!UICONTROL Field Name]:** El nombre del campo, preferiblemente escrito en camelCase. No se permiten caracteres de espacio. Es el nombre que se utiliza para hacer referencia al campo en el código y en otras aplicaciones de flujo descendente.
   * Ejemplo: loyaltyLevel
* **[!UICONTROL Display Name]:** El nombre del campo, escrito en mayúsculas y minúsculas. Este es el nombre que se mostrará en el lienzo al ver o editar el esquema.
   * Ejemplo: Nivel de fidelización
* **[!UICONTROL Type]:** El tipo de datos del campo. Esto incluye tipos escalares básicos y cualquier tipo de datos definido en [!DNL Schema Registry]. Ejemplos: [!UICONTROL String], [!UICONTROL Integer], [!UICONTROL Boolean], [!UICONTROL Person], [!UICONTROL Address], [!UICONTROL Phone number], etc.
* **[!UICONTROL Description]:** Se debe incluir una descripción opcional del campo con un máximo de 200 caracteres.
...
Dado que `id` será una cadena de forma libre generada aleatoriamente, no se necesitan más restricciones. Seleccione **[!UICONTROL Apply]** para aplicar los cambios.
...
| [!UICONTROL Enum & Suggested Values] | Una enumeración indica que este campo debe contener uno de los valores de una lista enumerada de valores posibles. También puede utilizar esta opción para proporcionar una lista de valores sugeridos para un campo de cadena sin restringir el campo a esos valores. |
| [!UICONTROL Identity] | Indica que este campo es un campo de identidad. Se proporciona más información sobre los campos de identidad [más adelante en este tutorial](#identity-field). |
| [!UICONTROL Relationship] | Aunque las relaciones de esquema se pueden inferir mediante el uso del esquema de unión y [!DNL Real-Time Customer Profile], esto sólo se aplica a los esquemas que comparten la misma clase. La restricción [!UICONTROL Relationship] indica que este campo hace referencia a la identidad principal de un esquema basado en una clase diferente, lo que implica una relación entre los dos esquemas. Vea el tutorial sobre [definición de una relación](./relationship-ui.md) para obtener más información. |
...
![Editor de esquemas con el objeto de clase de nivel agregado y resaltado en [!UICONTROL Field properties].](../images/tutorials/create-schema/tier-class-type.png)

Aparecen casillas de verificación adicionales para el campo después de seleccionar su tipo, incluidas las casillas de verificación de **[!UICONTROL Array]**, **[!UICONTROL Enum & Suggested Values]**, **[!UICONTROL Identity]** y **[!UICONTROL Relationship]**.

Seleccione la casilla de verificación **[!UICONTROL Enum & Suggested Values]** y luego seleccione **[!UICONTROL Enum]**. Aquí puede escribir **[!UICONTROL Value]** (en camelCase) y **[!UICONTROL Display Name]** (un nombre opcional y de fácil lectura en Title Case) para cada clase de nivel de fidelidad aceptable.

Cuando haya completado todas las propiedades del campo, seleccione **[!UICONTROL Apply]** para agregar el campo `tierClass` al objeto `loyaltyTier`.

![Las propiedades del campo de enumeración y sugerencia de valores se completaron con [!UICONTROL Apply] resaltadas.](../images/tutorials/create-schema/tier-class-enum.png)
...
Para convertir el objeto `loyaltyTier` en un tipo de datos, seleccione el campo `loyaltyTier` en el lienzo y, a continuación, seleccione **[!UICONTROL Convert to new data type]** a la derecha del editor en **[!UICONTROL Field properties]**.

![Editor de esquemas con el objeto loyaltyTier y [!UICONTROL Convert to new data type] resaltado.](../images/tutorials/create-schema/convert-data-type.png)
...
La estructura de datos estándar que proporcionan los esquemas se puede aprovechar para identificar datos que pertenecen a la misma persona en varias fuentes, lo que permite varios casos de uso descendentes, como segmentación, creación de informes, análisis de ciencia de datos y más. Para unir datos basados en identidades individuales, los campos clave deben marcarse como [!UICONTROL Identity] campos dentro de los esquemas aplicables.

[!DNL Experience Platform] facilita la denotación de un campo de identidad mediante el uso de una casilla de verificación **[!UICONTROL Identity]** en [!DNL Schema Editor]. Sin embargo, debe determinar qué campo es el mejor candidato para utilizar como identidad, según la naturaleza de los datos.

Por ejemplo, puede haber miles de miembros del programa de fidelidad que pertenecen al mismo nivel de fidelidad y varios que comparten la misma dirección física. Sin embargo, en esta situación, al inscribirse, cada miembro del programa de fidelidad proporciona su dirección de correo electrónico personal. Dado que las direcciones de correo electrónico personales normalmente las administra una persona, el campo `personalEmail.address` (proporcionado por el grupo de campos [!UICONTROL Personal Contact Details]) es un buen candidato para un campo de identidad.
...
Seleccione el campo `personalEmail.address` en el lienzo y la casilla de verificación **[!UICONTROL Identity]** aparecerá bajo **[!UICONTROL Field properties]**. Marque la casilla y aparecerá la opción para establecer esto como **[!UICONTROL Primary identity]**. Seleccione también esta casilla.
...
Después de aplicar el cambio, el icono de `personalEmail.address` muestra un símbolo de huella digital, que indica que ahora es un campo de identidad. El campo también aparece en el carril izquierdo bajo **[!UICONTROL Identities]**.
...
Seleccione **[!UICONTROL Profile]** y aparecerá una ventana emergente, pidiéndole que confirme que desea habilitar el esquema para [!DNL Profile].
...
Seleccione **[!UICONTROL Enable]** para confirmar su elección. Puede volver a seleccionar el conmutador **[!UICONTROL Profile]** para deshabilitar el esquema si lo desea, pero una vez guardado el esquema mientras [!DNL Profile] está habilitado, ya no se puede deshabilitar.
...
Se puede eliminar un esquema en la interfaz de usuario desde el Editor de esquemas mediante acciones [!UICONTROL More] y también desde los detalles del esquema en la pestaña [!UICONTROL Browse]. Existen ciertas condiciones que impiden que se elimine un esquema. Un esquema no se puede eliminar si:
...
Seleccione **[!UICONTROL Copy JSON structure]** para generar una carga útil de exportación para cualquier esquema de la biblioteca de esquemas. Esta acción copia la estructura JSON en el portapapeles. El JSON exportado se puede utilizar para importar el esquema y cualquier recurso relacionado en una zona protegida u organización diferente. Esto hace que compartir y reutilizar esquemas entre diferentes entornos sea sencillo y eficaz.

## Pasos siguientes y recursos adicionales

Ahora que ha terminado de maquetar el esquema, puede ver el esquema completo en el lienzo. Seleccione **[!UICONTROL Save]** y el esquema se guardará en [!DNL Schema Library], para que el [!DNL Schema Registry] pueda obtener acceso a él.
