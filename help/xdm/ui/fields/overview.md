...
Una vez que tenga la variable [!DNL Schema Editor] abrir, los controles para añadir campos aparecen en el lienzo. Estos controles aparecen junto al nombre del esquema, así como cualquier campo de tipo de objeto que se haya definido en la clase o grupo de campos seleccionados.
...
| [!UICONTROL Nombre para mostrar] | Un nombre para mostrar para el campo. Este es el nombre que se utilizará para representar el campo dentro del lienzo del Editor de esquemas. El nombre del campo se puede cambiar por el nombre para mostrar mediante la función [alternador de nombre para mostrar](../resources/schemas.md#display-name-toggle). |
