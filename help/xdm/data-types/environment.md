...
[!UICONTROL Entorno] es un tipo de datos XDM estándar que describe el entorno de un evento observado, con información transitoria específica, como las versiones de red y software.
...
| --- | --- | --- |
| `_dc` | Objeto | Un objeto que contiene un solo campo, `language`, que indica el idioma del entorno para representar las preferencias lingüísticas, geográficas o culturales del usuario en cuanto a la presentación de datos. Los idiomas se especifican en el código de idioma tal como se define en [IETF RFC 3066](https://www.ietf.org/rfc/rfc3066.txt). |
...
| `colorDepth` | Número entero | El número de bits utilizados para cada componente de color de un solo píxel. |
...
| `operatingSystem` | Cadena | El nombre del sistema operativo utilizado cuando se realizó la observación. El atributo no debe contener información de versión, como `10.5.3`, pero en su lugar contienen designaciones de &quot;edición&quot; como `Ultimate` o `Professional`. |
| `operatingSystemVendor` | Cadena | El nombre del proveedor del sistema operativo utilizado cuando se realizó la observación. |
...
| `type` | Cadena | El tipo de entorno de la aplicación. Consulte la [apéndice](#type) para valores aceptados. |
| `viewportHeight` | Número entero | El tamaño vertical en píxeles de la ventana en la que se mostró la experiencia. En el caso de los eventos de visualización web, esta es la altura de la ventanilla del explorador. |
| `viewPortWidth` | Número entero | El tamaño horizontal en píxeles de la ventana en la que se mostró la experiencia. En un evento de visualización web, esta es la anchura de la ventanilla del explorador. |
