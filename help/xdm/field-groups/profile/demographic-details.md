...
[!UICONTROL Detalles demográficos] es un grupo de campos de esquema estándar para la [[!DNL XDM Individual Profile] clase](../../classes/individual-profile.md). El grupo de campos proporciona un objeto `person` de nivel raíz, cuyos subcampos describen información sobre una persona individual.
...
| `person.birthYear` | Entero | El año en que nació una persona, incluido el siglo (como 1989). Este campo debe usarse cuando solo se conoce la edad de la persona, no la fecha de nacimiento completa. |
| `person.gender` | Cadena | La identidad de género de la persona. |
| `person.martialStatus` | Cadena | Hace referencia a la relación de una persona con su pareja. |
