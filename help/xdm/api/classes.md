...
>[!NOTE]
>
>Para obtener más información sobre las clases de comportamiento de los datos en cuanto a cómo afectan a la composición del esquema, consulte los [conceptos básicos de la composición](../schema/composition.md)del esquema.

El `/classes` punto final de la [!DNL Schema Registry] API le permite administrar clases mediante programación dentro de la aplicación de experiencia.

## Primeros pasos

El punto final utilizado en esta guía forma parte de la [[!DNL Schema Registry] API](https://www.adobe.io/apis/experienceplatform/home/api-reference.html#!acpdr/swagger-specs/class-registry.yaml). Antes de continuar, consulte la guía [de](./getting-started.md) introducción para ver los vínculos a la documentación relacionada, una guía para leer las llamadas de la API de muestra en este documento e información importante sobre los encabezados necesarios para realizar llamadas correctamente a cualquier API de Experience Platform.

## Recuperar una lista de clases {#list}

Puede realizar la lista de todas las clases bajo el `global` contenedor o `tenant` realizando una solicitud de GET a `/global/classes` o `/tenant/classes`, respectivamente.
...
| --- | --- |
| `{CONTAINER_ID}` | El contenedor desde el que desea recuperar las clases: `global` para clases creadas por Adobes o `tenant` para clases que son propiedad de su organización. |
...
La siguiente solicitud recupera una lista de clases del `tenant` contenedor, utilizando un parámetro de `orderby` consulta para ordenar las clases por su `title` atributo.
...
La solicitud anterior utilizó el `application/vnd.adobe.xed-id+json` encabezado, por lo tanto la respuesta incluye solamente los atributos `Accept` , `title`, `$id`y `meta:altId``version` de cada clase. El uso del otro `Accept` encabezado (`application/vnd.adobe.xed+json`) devuelve todos los atributos de cada clase. Seleccione el `Accept` encabezado correspondiente en función de la información que necesite en la respuesta.
...
| Parámetro | Descripción |
| --- | --- |
| `{CONTAINER_ID}` | El contenedor que aloja la clase que desea recuperar: `global` para una clase creada por un Adobe o `tenant` para una clase propiedad de su organización. |
| `{CLASS_ID}` | Codificación `meta:altId` o URL `$id` de la clase que desea buscar. |

**Solicitud**

La siguiente solicitud recupera una clase según el `meta:altId` valor proporcionado en la ruta.
...
Realizar una solicitud de GET para [lista de todas las clases](#list) del `tenant` contenedor ahora incluiría la clase Property. También puede [realizar una solicitud](#lookup) de búsqueda (GET) utilizando la codificación de URL `$id` para vista directa de la nueva clase.

## Actualizar una clase {#put}

Puede reemplazar una clase completa mediante una operación de PUT, lo que básicamente es volver a escribir el recurso. Al actualizar una clase a través de una solicitud de PUT, el cuerpo debe incluir todos los campos que serían necesarios al [crear una nueva clase](#create) en una solicitud de POST.

>[!NOTE]
>
>Si solo desea actualizar parte de una clase en lugar de reemplazarla por completo, consulte la sección sobre la [actualización de una parte de una clase](#patch).
...
| Parámetro | Descripción |
| --- | --- |
| `{CLASS_ID}` | Codificación `meta:altId` o URL `$id` de la clase que desea volver a escribir. |

**Solicitud**

La siguiente solicitud vuelve a escribir una clase existente, cambiando su valor `description` y el `title` de uno de sus campos.
...
Puede actualizar una parte de una clase mediante una solicitud de PATCH. El [!DNL Schema Registry] es compatible con todas las operaciones estándar de Parche JSON, incluidas `add`, `remove`y `replace`. Para obtener más información sobre JSON Patch, consulte la guía [de aspectos fundamentales de la API](../../landing/api-fundamentals.md#json-patch).

>[!NOTE]
>
>Si desea reemplazar un recurso completo con nuevos valores en lugar de actualizar campos individuales, consulte la sección sobre [reemplazar una clase mediante una operación](#put)de PUT.
...
| Parámetro | Descripción |
| --- | --- |
| `{CLASS_ID}` | El `$id` URI con codificación URL o `meta:altId` de la clase que desea actualizar. |
...
La respuesta muestra que ambas operaciones se realizaron correctamente. El `description` se ha actualizado, junto con el `title` del `propertyId` campo.
...
| Parámetro | Descripción |
| --- | --- |
| `{CLASS_ID}` | El `$id` URI con codificación URL o `meta:altId` de la clase que desea eliminar. |
...
Para confirmar la eliminación, intente una solicitud [de](#lookup) búsqueda (GET) para la clase. Deberá incluir un `Accept` encabezado en la solicitud, pero deberá recibir un estado HTTP 404 (no encontrado) porque la clase se ha eliminado del Registro de Esquemas.