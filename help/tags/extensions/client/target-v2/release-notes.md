...
- Se actualizó para admitir `at.js` 2.11.4
...
- Se actualizó para admitir `at.js` 2.11.3
- Se ha corregido un error que impedía que se enviaran tokens de respuesta en eventos at-content-rendering-failed.

## v0.20.1 (3 de noviembre de 2023)

- Se ha actualizado para admitir `at.js` 2.11.2.
...
- Se ha actualizado para admitir `at.js` 2.11.0.
...
- Se ha actualizado para admitir `at.js` v2.10.1
...
- Se ha corregido que `pageLoad` no se asignara a `target-global-mbox` en el modo de ejecución híbrida impar
- Se corrigió un problema con los detalles de análisis para la solicitud `mbox`
- Dependencias de desarrollo actualizadas para corregir vulnerabilidades de seguridad

## v0.17.0 (7 de enero de 2022)

- Se ha actualizado para admitir `at.js` v2.8.0, que ahora recopila datos de uso de características y telemetría de rendimiento.  Los datos personales no se recopilan. Para no participar en esta función, establezca `telemetryEnabled` en `false` en `targetGlobalSettings`.

## v0.16.0 (28 de octubre de 2021)

- Se ha actualizado para admitir `at.js` v2.7.0, que ahora está disponible para su descarga en Adobe Target.

## v0.15.2 (16 de agosto de 2021)

- Se ha actualizado para admitir `at.js` 2.6.1.
...
- Agregar atributo seguro a las cookies cada vez que la configuración de `at.js` secureOnly se establezca en verdadera
...
- Se ha corregido un error por el que el paquete final contenía dos versiones de `at.js`, una con Toma de decisiones en el dispositivo y otra sin esta opción.
...
- Se agregó una nueva acción Load Target con [On-Device Decisioning](./overview.md#load-target-with-on-device-decisioning), que carga `at.js` 2.5 con capacidades de On-Device Decisioning
- Se actualizó `at.js` a 2.5
...
- Se ha actualizado `at.js` a 2.4.1.
...
- Se ha actualizado la versión de `at.js` a 2.3.3
...
- Se ha corregido un problema que se producía al usar CNAME y la anulación de perímetros, por el cual `at.js` 1.x podría crear incorrectamente el dominio del servidor, lo que provocaba un error en la solicitud de Target
...
- Anteriormente, la biblioteca ECID realizaba dos llamadas de bloqueo antes de que `at.js` pudiera recuperar experiencias. Esto se ha reducido a una sola llamada, lo que mejora significativamente el rendimiento.
