...
* **Corrección de** erroresSe ha corregido un problema por el que una regla que utilizaba un evento Page Bottom y una acción de Custom Code en una página en la que las etiquetas se cargaban sincrónica pero incorrectamente (sin llamada a  `_satellite.pageBottom()`) borraba el contenido del sitio web.
* **Corrección de** erroresSe ha corregido un problema en el que Enters Viewport no funcionaba si la biblioteca de etiquetas se cargaba de forma asíncrona y terminaba de cargarse después de activarse el evento DOMContentLoaded del explorador.
...
* Se ha corregido un problema por el cual algunos botones de las extensiones de etiquetas no se espaciaban correctamente.
...
¡Las etiquetas se han hecho móviles!

Dado que miles de clientes lo utilizan para la administración de etiquetas web, el espacio de trabajo de etiquetas ahora se puede utilizar para administrar los SDK móviles.

La creación de una propiedad móvil en la interfaz de usuario de la recopilación de datos permite:
...
* Administrar extensiones móviles:
   ...
   * Las extensiones están disponibles para otras soluciones de Adobe, incluidas [!DNL Adobe Analytics], [!DNL Adobe Target] y [!DNL Adobe Audience Manager].
...
Para obtener más información sobre cómo empezar a utilizar las propiedades móviles, consulte [Mobile](https://aep-sdks.gitbook.io/docs/).
...
Las propiedades de etiqueta creadas recientemente ahora incluyen un host [!DNL "Managed by Adobe"] y tres entornos (uno de cada tipo) de forma predeterminada.
...
* En algunos casos, las reglas guardadas a través de la interfaz de usuario no se guardaban en el orden correcto.  Esto se ha solucionado. Está previsto que, en una actualización futura, una migración de datos corrija todas las reglas afectadas, pero mientras tanto se puede corregir la regla afectada editándola manualmente, aplicándole un cambio y guardándola.
...
#### Las etiquetas y las bibliotecas de DTM ya están activas en China

Las etiquetas y las bibliotecas [!DNL DTM] que utilizan [!DNL Akamai] ya están disponibles en los nodos perimetrales de CDN en China. Esto proporciona velocidades de carga de biblioteca mucho más rápidas para los usuarios finales en China.
...
La documentación de código abierto para las etiquetas está disponible en [Experience League](https://experienceleague.adobe.com/docs/launch/using/overview.html?lang=es).
...
El ID de regla ahora se emite para cada regla de una compilación y se puede hacer referencia a ella en el explorador.
...
#### Vincular código incrustado de DTM a etiquetas

Al vincular el código incrustado de [!DNL DTM] a las etiquetas, puede mantener el código incrustado de producción [!DNL DTM] en una página, pero servir archivos de etiqueta en lugar de [!DNL DTM].
...
* Crear una nueva biblioteca desde el menú desplegable Biblioteca activa
...
* Compilaciones de ensayo de etiquetas sin &quot;-staging&quot; en el nombre de archivo: Se hereda el valor predeterminado de 60 minutos
...
>[!IMPORTANT]
>
>Los desarrolladores de etiquetas que no tengan `-development` o `-staging` en sus códigos incrustados de entorno deben volver a crear los entornos de desarrollo y ensayo para obtener el encabezado de control de caché de 0. Si no vuelve a crear los entornos, se utiliza el mismo control de caché de 60 minutos que en las bibliotecas de producción.
...
* Page Top ahora se llama Library Loaded
* On Load ahora se llama Window Loaded

### Correcciones de errores y mejoras

* Se ha corregido un problema por el cual la biblioteca de etiquetas se cargaba dos veces en determinados casos puntuales.
...
Esta es la primera versión de etiquetas en Adobe Experience Platform.

Etiquetas es la función de administración de etiquetas de próxima generación que genera el Adobe. Ofrece a los clientes una alternativa sencilla para implementar y gestionar todas las etiquetas de análisis, marketing y publicidad necesarias para ofrecer al cliente experiencias más relevantes.

Las etiquetas permiten a los usuarios crear y mantener sus propias integraciones mediante extensiones. Los clientes de Platform pueden encontrar estas extensiones en las tiendas de aplicaciones para instalar, configurar e implementar rápidamente sus etiquetas.

Las etiquetas se proporcionan a los clientes de Adobe Experience Cloud como una función incluida que añade valor. Las etiquetas son un producto completamente nuevo con un nuevo código base, diseñado para reemplazar al servicio [!DNL Dynamic Tag Management (DTM)] anterior. Sin embargo, se prevé que el servicio mantenga la compatibilidad con [!DNL DTM]. [!DNL Adobe] seguirá corrigiendo cualquier error significativo y garantizará que funciona de forma coherente. En este momento no se planifica realizar mejoras de las características principales de las versiones anteriores de [!DNL DTM].
...
Una extensión es un paquete de código (JavaScript, HTML y CSS) que amplía la funcionalidad del cliente y la interfaz de usuario de recopilación de datos. &#x200B;Cree, gestione y actualice sus integraciones con una interfaz de autoservicio virtual. Puede considerar una etiqueta como un sistema operativo y las extensiones son las aplicaciones que utiliza para realizar sus tareas.
