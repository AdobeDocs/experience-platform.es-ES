...
Cuando se produce un problema al realizar una llamada a la API de Reactor, puede devolverse un error de una de las siguientes maneras:
...
| `code` | Código de error específico de la aplicación, expresado como valor de cadena. |
| `title` | Un breve resumen legible por el usuario del problema que **no debería cambiar** de ocurrencia a ocurrencia, excepto para fines de localización. |
...
| --- | --- |
| `authentication-failure` | El token de acceso de IMS no es válido. Para obtener un nuevo token de acceso, vuelva a iniciar sesión. O para cuentas técnicas, generando un nuevo JWT e intercambiando por un token de acceso IMS. |
| `connection-refused` | No se pudo establecer una conexión con el servidor. |
| `decrypt-bad-passphrase` | No se pudieron descifrar los datos con la frase de contraseña proporcionada. |
| `decrypt-failed` | No se pudieron descifrar los datos con la clave privada proporcionada. Asegúrese de que la clave funciona localmente y que se han recortado los espacios en blanco. |
| `decrypt-no-data` | Los datos no se pueden descifrar sin una clave privada. Proporcione una clave privada cifrada. |
| `delegate-descriptor-unresolved` | La extensión no proporcionó la definición esperada de este descriptor delegado. Puede que sea necesario actualizar la extensión. |
| `deleted-resources` | Se han eliminado los recursos que intenta agregar a la biblioteca. |
| `environment-in-use` | Un entorno solo se puede asignar a una biblioteca a la vez. La opción 1 es elegir un entorno diferente. La opción 2 es liberar este entorno moviendo la biblioteca a otro entorno o eliminando la biblioteca. |
| `environment-required` | La biblioteca debe tener un entorno asignado para poder crear una compilación. |
| `extension-not-found` | La extensión que define un elemento de datos o un componente de regla no se incluye en la biblioteca de . Asegúrese de que todas las extensiones requeridas se hayan añadido a la biblioteca de . |
| `extension-package-path-error` | Una ruta definida en extension.json se construyó incorrectamente. |
...
| `ims-session-error` | Hay un problema con la sesión de inicio de sesión. Cierre la sesión y vuelva a iniciarla. |
...
| `invalid-data_element` | No se puede agregar un elemento de datos no válido a una biblioteca. |
| `invalid-embed_code` | No se trata de un código incrustado válido o está intentando vincularlo a un entorno de ensayo o desarrollo. Los códigos incrustados de Dynamic Tag Management (DTM) solo se pueden vincular a entornos de producción. |
| `invalid-extension` | No se puede agregar una extensión no válida a una biblioteca. |
| `invalid-extension_package_id` | Solo puede modificar algunas de las propiedades de objeto de un paquete de extensión. Intentó modificar uno de los que no está permitido. |
...
| `invalid-rule` | No se puede agregar una regla no válida a una biblioteca. |
...
| `minification-error` | No se pudo compilar el código debido a código no válido o código ES6. |
| `multiple-revisions` | En una biblioteca solo se puede incluir una revisión de cada recurso. |
| `no-available-orgs` | Esta cuenta de usuario no pertenece a ningún perfil de producto que tenga acceso a etiquetas. Utilice el Admin Console para agregar este usuario a un perfil de producto con derechos de etiquetas. |
| `not-authorized` | Esta cuenta de usuario no tiene los permisos necesarios para realizar esta acción. |
| `not-found` | No se pudo encontrar el registro. Compruebe el id del objeto que está intentando recuperar. |
| `not-unique` | El nombre que intenta usar ya está en uso. Para este recurso, la propiedad &quot;name&quot; debe ser única. |
| `public-release-not-authorized` | La publicación pública de extensiones está coordinada por `launch-ext-dev@adobe.com`. Para obtener más información, consulte el documento sobre [extensión](../../extension-dev/submit/release.md). |
...
| `sftp-exception` | Se encontró una excepción al usar SFTP para conectarse al servidor. |
...
| `upgraded-extension-required` | Una vez que haya instalado una actualización de extensión, debe incluirla en todas las bibliotecas hasta que la actualización llegue a Producción. La única excepción es si la extensión aún no se ha publicado. |
