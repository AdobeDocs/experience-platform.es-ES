...
A partir de la versión 2.15.0, el SDK web captura la variable `region` del elemento HTML en el que se hizo clic. Esto habilita la variable [Activity Map](https://experienceleague.adobe.com/docs/analytics/analyze/activity-map/activity-map.html?lang=es) funciones de informes en Adobe Analytics.
...
Los datos recopilados con el seguimiento automático de vínculos pueden inspeccionarse y filtrarse proporcionando un [Función de llamada de retorno onBeforeEventSend](../fundamentals/tracking-events.md#modifying-events-globally).
...
A partir de la versión 2.15.0 del SDK web, los datos recopilados con el seguimiento automático de vínculos se pueden inspeccionar, aumentar o filtrar proporcionando un [Función de llamada de retorno onBeforeLinkClickSend](../fundamentals/configuring-the-sdk.md#onBeforeLinkClickSend).

Esta función de llamada de retorno solo se ejecuta cuando se produce un evento de clic en vínculo automático.
...
Al filtrar los eventos de seguimiento de vínculos mediante la variable `onBeforeLinkClickSend` comando, Adobe recomienda devolver `false` para los clics en vínculos de los que no debe realizarse un seguimiento. Cualquier otra respuesta hará que el SDK web envíe los datos a la red perimetral.


>[!NOTE]
>
>** Cuando ambas variables `onBeforeEventSend` y `onBeforeLinkClickSend` las funciones de llamada de retorno están establecidas, el SDK web ejecuta el `onBeforeLinkClickSend` función de llamada de retorno para filtrar y aumentar el evento de interacción de clic en vínculo, seguido del `onBeforeEventSend` función de llamada de retorno.