...
| `1104-504` | Tiempo de espera de puerta | Se agotó el tiempo de espera de puerta de enlace. Inténtelo de nuevo. Si el problema persiste, póngase en contacto con el servicio de atención al cliente. |
...
| `1401-400` | Solicitud no válida | Los parámetros limit y count no se pueden combinar en la misma solicitud. Proporcione solo el límite o el parámetro de recuento e inténtelo de nuevo. |
...
| `1404-412` | La versión no coincide | La versión &quot;v1&quot; proporcionada no coincide con la versión actual en la entidad &quot;cc01fc2c-0000-0200&quot;. Asegúrese de que la versión proporcionada coincida con la versión actual en la entidad e inténtelo de nuevo. |
...
| `1407-400` | Solicitud no válida | El estado fallido no se puede utilizar con `progress` subOp. Elimine el `progress` subOp o use el status=&#39;inProgress&#39; e inténtelo de nuevo. |
| `1408-400` | Solicitud no válida | El porcentaje completado debe ser 100 para los estados completado o suspendido. Actualice el porcentaje completado e inténtelo de nuevo. |
...
| `1416-400` | Solicitud no válida | Eliminar o actualizar solo se puede aplicar en una conexión con estado habilitado, deshabilitado o de inicialización. |
| `1417-400` | Solicitud no válida | Eliminar o actualizar conexiones en `initializing` No se permiten los estados con UserToken. |
...
| `1422-400` | Solicitud no válida | La acción deshabilitada no se puede aplicar en el estado actual {state}. Actualice la acción e inténtelo de nuevo. |
...
| `1424-400` | Solicitud no válida | OrderBy no es compatible con consultas entre zonas protegidas. |
...
| `1426-400` | Solicitud no válida | El token de usuario no está autorizado para crear/actualizar la especificación de conexión. Asegúrese de que el token de usuario esté autorizado e inténtelo de nuevo. |
| `1427-400` | Solicitud no válida | El token de usuario no está autorizado para crear/actualizar ejecuciones de flujo. Asegúrese de que el token de usuario esté autorizado e inténtelo de nuevo. |
...
| `1431-400` | Solicitud no válida | La directiva no permite campos de matriz vacíos. |
| `1432-400` | Solicitud no válida | El orden especificado no es correcto, anteponga + para orden ascendente y - para orden descendente en fieldName. |
...
| `1445-400` | Solicitud no válida | Función de conexión de PATCH no habilitada. |
...
| `1456-400` | Solicitud no válida | La eliminación-finalización del estado no es un valor válido. Los valores permitidos son [habilitado, deshabilitado, inicializando]. |
...
| `2106-404` | Error al obtener zona protegida predeterminada | No se ha encontrado la información predeterminada de la zona protegida. |
...
| `2112-400` | No se permiten encabezados con un valor diferente | Los encabezados x-sandbox-name y x-sandbox-id deben tener el valor * para la consulta entre zonas protegidas. Actualice los encabezados e inténtelo de nuevo. |
...
| `2116-500` | Error interno | Se produjo un error interno durante la descodificación sin conexión del token de portador para la resolución del ámbito. |
...
| `2120-403` | Prohibido | No tiene permisos suficientes para realizar la operación. Póngase en contacto con el administrador para resolver los permisos e inténtelo de nuevo. |
| `2121-403` | Prohibido | El administrador de permisos se ha denegado en el recurso Origen de empresa. Póngase en contacto con el administrador para resolver los permisos e inténtelo de nuevo. |
| `2200-500` | Error de solicitud de servicio externo | Se ha producido un error al llamar al servicio de catálogo para validar el esquema. |
| `2250-500` | Error de solicitud de servicio externo | Se ha producido un error al llamar al servicio de preparación de datos para validar la asignación. |
