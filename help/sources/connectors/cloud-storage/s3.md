...
Para ingerir un solo directorio con S3, debe crear un usuario [!DNL Identity and Access Management] (IAM) para Platform en la consola S3 y asignar permisos para las siguientes acciones:
...
Una ruta de archivo como `myBucket/folder/subfolder/subsubfolder/abc.csv` puede llevar solo al acceso `subsubfolder/abc.csv`. Si desea acceder a la subcarpeta, puede especificar el parámetro `bucket` en la consola S3 como `myBucket` y el `folderPath` como `folder/subfolder` para asegurarse de que la exploración del archivo comience en `subfolder` en lugar de en `subsubfolder/abc.csv`.
