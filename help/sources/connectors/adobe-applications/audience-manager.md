...
- **Datos de perfil:** Audience Manager utiliza datos en tiempo real e incorporados para derivar perfiles de clientes. Estos perfiles se utilizan para rellenar gráficos de identidad y rasgos en las realizaciones de segmentos.
...
Para obtener más información, lea la guía de [crear una conexión de origen de Audience Manager en la interfaz de usuario](../../tutorials/ui/create/adobe-applications/audience-manager.md).
...
Para obtener más información sobre cómo se utiliza XDM en Experience Platform, lea la [Información general del sistema XDM](../../../xdm/home.md). Para obtener más información sobre cómo se estructuran los esquemas XDM entre perfiles y eventos, lea la [conceptos básicos de composición de esquemas](../../../xdm/schema/composition.md).
...
>[!NOTE]
>
>AAM El tiempo real es el único conjunto de datos que va al lago de datos de. Todos los demás conjuntos de datos de Audience Manager se dirigen a [!DNL Profile], si están habilitadas para [!DNL Profile]. Si no están habilitados para [!DNL Profile], no reciben ningún dato y se mostrarán como vacíos.
...
| AAM Actualizaciones de perfil en tiempo real de | Este conjunto de datos permite la segmentación en tiempo real de rasgos y segmentos de Audience Manager. Incluye información sobre el enrutamiento regional de Edge, las características y la pertenencia a segmentos. Mantenga este conjunto de datos habilitado para la ingesta de perfiles. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alternar, para introducir directamente los datos en el perfil. | Registro |
| AAM Datos de dispositivos | Datos del dispositivo con ECID y las realizaciones de segmentos correspondientes acumulados en Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alternar, para introducir directamente los datos en el perfil. | Registro |
| AAM Datos de perfil de dispositivo de | Se utiliza para los diagnósticos del conector del Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alternar, para introducir directamente los datos en el perfil. | Registro |
| AAM Perfiles autenticados de la | Este conjunto de datos contiene perfiles autenticados por el Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alternar, para introducir directamente los datos en el perfil. | Registro |
| AAM Metadatos de perfiles autenticados de | Se utiliza para los diagnósticos del conector del Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alternar, para introducir directamente los datos en el perfil. | Registro |
| AAM Relleno de datos de dispositivos | El conjunto de datos no trae datos de dispositivos anteriores. Contiene ECID y las realizaciones de segmentos correspondientes agregadas en Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alterne para introducir directamente los datos en el perfil. | Registro |
| AAM Relleno de perfiles autenticados | Que el conjunto de datos traiga datos autenticados en el pasado. Contiene perfiles autenticados por el Audience Manager. Los datos no son visibles como lotes en el conjunto de datos. Puede activar la variable **[!UICONTROL Perfil]** alterne para introducir directamente los datos en el perfil. | Registro |

### Conexiones

Adobe Audience Manager crea una conexión en el catálogo: Conexión del Audience Manager. El catálogo es el sistema de registros para la ubicación y el linaje de los datos dentro de Adobe Experience Platform. Una conexión es un objeto Catalog que es una instancia de conectores específica del cliente. Lea el [Resumen del servicio de catálogo](../../../catalog/home.md) para obtener más información sobre Catálogo, conexiones y conectores.
...
También puede comprobar el uso del perfil que tiene disponible su cuenta a través del [Tablero de uso de licencias](../../../dashboards/guides/license-usage.md).
...
| Datos en tiempo real e incorporados | Actualizaciones de perfil | de 24 a 36 horas | El tiempo transcurre desde que se captura mediante datos perimetrales DCS/PCS e incorporados, se procesa en un perfil de usuario y, finalmente, aparece en el Perfil del cliente en tiempo real. Actualmente, estos datos no aterrizan directamente en el lago de datos. Se puede habilitar la opción de perfil para que los conjuntos de datos de perfil del Audience Manager ingieran estos datos directamente en el perfil del cliente en tiempo real. |
