---
title: Cree una conexión de origen y un flujo de datos para entidades NetSuite de Oracle mediante la API de Flow Service
description: Aprenda a crear una conexión de origen y un flujo de datos para llevar los contactos de NetSuite de Oracle y los datos de clientes a Experience Platform mediante la API de Flow Service.
hide: true
hidefromtoc: true
badge: Beta
...
# Crear una conexión de origen y un flujo de datos para [!DNL Oracle NetSuite Entities] uso de la API de Flow Service
...
Lea el siguiente tutorial para aprender a obtener contactos y datos de clientes de su [!DNL Oracle NetSuite Activities Entities] a Adobe Experience Platform mediante el [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Las secciones siguientes proporcionan información adicional que deberá conocer para conectarse correctamente a [!DNL Oracle NetSuite Entities] uso del [!DNL Flow Service] API.
...
## Connect [!DNL Oracle NetSuite Entities] a Platform mediante el [!DNL Flow Service] API

A continuación se describen los pasos que debe seguir para autenticar su [!DNL Oracle NetSuite Entities] origen, crear una conexión de origen y crear un flujo de datos para llevar los datos de cliente y de contacto a Experience Platform.

### Crear una conexión base {#base-connection}

Una conexión base retiene información entre el origen y Platform, incluidas las credenciales de autenticación del origen, el estado actual de la conexión y el ID único de conexión base. El ID de conexión base le permite explorar y navegar por archivos desde el origen e identificar los elementos específicos que desea introducir, incluida la información sobre sus tipos de datos y formatos.

Para crear un ID de conexión base, realice una solicitud de POST al `/connections` extremo al proporcionar su [!DNL Oracle NetSuite Entities] credenciales de autenticación como parte del cuerpo de la solicitud.
...
| `auth.params.accessTokenUrl` | El [!DNL NetSuite] URL de token de acceso, similar a `https://{ACCOUNT_ID}.suitetalk.api.netsuite.com/services/rest/auth/oauth2/v1/token` donde reemplazará ACCOUNT_ID por su [!DNL NetSuite] ID de cuenta. |
| `auth.params.accessToken` | El valor del token de acceso se genera al final del [paso dos](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158081952044.html#Step-Two-POST-Request-to-the-Token-Endpoint) de la [Flujo de concesión de código de autorización de OAuth 2.0](https://docs.oracle.com/en/cloud/saas/netsuite/ns-online-help/section_158074210415.html#OAuth-2.0-Authorization-Code-Grant-Flow) tutorial. Los tokens de acceso caducan y solo son válidos durante 60 minutos. el valor es una cadena de 1024 caracteres con formato de token web JSON (JWT) similar a `eyJr......f4V0`. |
...
| `{SOURCE_PARAMS}` | Define parámetros para el archivo de origen que desea llevar a Platform. Para recuperar el tipo de formato aceptado para `{SOURCE_PARAMS}`, debe codificar toda la cadena en base64. <br> [!DNL Oracle NetSuite Entities] admite la recuperación de datos de clientes y contactos. Según el tipo de objeto que esté aprovechando, pase uno de los siguientes: <ul><li>`customer` : recupere datos específicos del cliente, incluidos detalles como nombres de clientes, direcciones e identificadores clave.</li><li>`contact` : Recupere nombres de contactos, correos electrónicos, números de teléfono y cualquier campo personalizado relacionado con los contactos y asociado a los clientes.</li></ul> |

>[!BEGINTABS]

>[!TAB Cliente]

Para [!DNL Oracle NetSuite Entities], para recuperar los datos de contacto, el valor de `{SOURCE_PARAMS}` se pasa como `{"object_type":"customer"}`. Cuando se codifica en base64, equivale a `eyAib2JqZWN0X3R5cGUiOiAiY3VzdG9tZXIifQ%3D%3D` como se muestra a continuación.
...
Para [!DNL Oracle NetSuite Entities], para recuperar los datos de contacto, el valor de `{SOURCE_PARAMS}` se pasa como `{"object_type":"contact"}`. Cuando se codifica en base64, equivale a `eyAib2JqZWN0X3R5cGUiOiAiY29udGFjdCJ9` como se muestra a continuación.
...
Al recuperar los datos del cliente, `object_type` el valor de la propiedad debe ser `customer`.
...
Al recuperar los datos de contacto, `object_type` el valor de la propiedad debe ser `contact`.
...
| `baseConnectionId` | El ID de conexión base de [!DNL Oracle NetSuite Entities]. Este ID se generó en un paso anterior. |
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | El formato del [!DNL Oracle NetSuite Entities] datos que desea introducir. Actualmente, el único formato de datos admitido es `json`. |
| `object_type` | [!DNL Oracle NetSuite Entities] admite la recuperación de clientes y contactos. Dependiendo de la entidad que desee, pase una de las siguientes acciones: <ul><li>`customer` : recupere datos específicos del cliente, incluidos detalles como nombres de clientes, direcciones e identificadores clave.</li><li>`contact` : Recupere nombres de contactos, correos electrónicos, números de teléfono y cualquier campo personalizado relacionado con los contactos y asociado a los clientes.</li></ul> |
...
| `data.format` | El formato del [!DNL Oracle NetSuite Entities] datos que desea introducir. |
...
El último paso para obtener datos de [!DNL Oracle NetSuite Entities] a Platform es crear un flujo de datos. Por ahora, tiene preparados los siguientes valores obligatorios:
