...
# Crear un flujo de datos para [!DNL Mailchimp Members] mediante la API de Flow Service

El siguiente tutorial lo acompañará durante los pasos para crear una conexión de origen y un flujo de datos para llevar datos de [!DNL Mailchimp Members] a Platform mediante la [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Una vez que haya recuperado sus credenciales de autenticación [!DNL Mailchimp], ahora puede iniciar el proceso de creación del flujo de datos para llevar los datos de [!DNL Mailchimp Members] a Platform. El primer paso para crear un flujo de datos es crear una conexión base.
...
Para crear una conexión base de [!DNL Mailchimp] con autenticación básica, realice una solicitud de POST al extremo `/connections` de la API [!DNL Flow Service] y proporcione las credenciales para `authorizationTestUrl`, `username` y `password`.
...
Para crear una conexión base [!DNL Mailchimp] con el código de actualización de OAuth 2, realice una solicitud de POST al extremo `/connections` y proporcione las credenciales para `authorizationTestUrl` y `accessToken`.
...
>[!TIP]
>
>Para recuperar el tipo de formato aceptado para `{SOURCE_PARAMS}`, debe codificar toda la cadena `list_id` en base64. Por ejemplo, `"list_id": "10c097ca71"` codificado en base64 equivale a `eyJsaXN0SWQiOiIxMGMwOTdjYTcxIn0=`.
...
| `params.listId` | También conocido como ID de audiencia, el ID de lista [!DNL Mailchimp] permite la transferencia de datos de audiencia a otras integraciones. |
...
| `data.format` | Formato de los datos de [!DNL Mailchimp] que desea llevar a Platform. |
...
El último paso para llevar los datos de [!DNL Mailchimp] a Platform es crear un flujo de datos. Por ahora, tiene preparados los siguientes valores obligatorios:
