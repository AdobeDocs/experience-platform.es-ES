...
# Crear una conexión de origen y un flujo de datos para [!DNL SugarCRM Accounts & Contacts] mediante la API de Flow Service

El siguiente tutorial lo acompañará durante los pasos para crear una conexión de origen de [!DNL SugarCRM Accounts & Contacts] y un flujo de datos para llevar los datos de cuentas y contactos de [[!DNL SugarCRM]](https://www.sugarcrm.com/) a Adobe Experience Platform mediante la [[!DNL Flow Service] API](https://www.adobe.io/experience-platform-apis/references/flow-service/).
...
Para conectar [!DNL SugarCRM Accounts & Contacts] a Platform, debe proporcionar valores para las siguientes propiedades de conexión:
...
## Conectar [!DNL SugarCRM Accounts & Contacts] a la plataforma mediante la API [!DNL Flow Service]

A continuación se describen los pasos que debe seguir para autenticar el origen de [!DNL SugarCRM], crear una conexión de origen y crear un flujo de datos para llevar los datos de cuentas y contactos al Experience Platform.

### Crear una conexión base {#base-connection}

Una conexión base retiene información entre el origen y Platform, incluidas las credenciales de autenticación del origen, el estado actual de la conexión y el ID único de conexión base. El ID de conexión base le permite explorar y navegar por archivos desde el origen e identificar los elementos específicos que desea introducir, incluida la información sobre sus tipos de datos y formatos.

Para crear un identificador de conexión base, realice una solicitud de POST al extremo `/connections` y proporcione sus credenciales de autenticación [!DNL SugarCRM Accounts & Contacts] como parte del cuerpo de la solicitud.
...
[!DNL SugarCRM Accounts & Contacts] admite varias API. Según el tipo de objeto que utilice, la solicitud que se va a enviar es la siguiente:
...
Para la API de cuentas [!DNL SugarCRM], el valor de `{SOURCE_PARAMS}` se pasa como `{"object_type":"accounts"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImFjY291bnRzIn0=`, como se muestra a continuación.
...
Para la API de contactos de [!DNL SugarCRM], el valor de `{SOURCE_PARAMS}` se pasa como `{"object_type":"contacts"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=`, como se muestra a continuación.
...
Para la API de cuentas [!DNL SugarCRM], el valor de propiedad `object_type` debe ser `accounts`.
...
Para la API de contactos de [!DNL SugarCRM], el valor de la propiedad `object_type` debe ser `contacts`.
...
| `baseConnectionId` | Identificador de conexión base de [!DNL SugarCRM Accounts & Contacts]. Este ID se generó en un paso anterior. |
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | Formato de los datos de [!DNL SugarCRM Accounts & Contacts] que desea introducir. Actualmente, el único formato de datos compatible es `json`. |
...
| `data.format` | Formato de los datos de [!DNL SugarCRM Accounts & Contacts] que desea introducir. |
...
El último paso para llevar datos de [!DNL SugarCRM Accounts & Contacts] a Platform es crear un flujo de datos. Por ahora, tiene preparados los siguientes valores obligatorios:
