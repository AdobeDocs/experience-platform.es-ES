---
title: Cree una conexión de origen y un flujo de datos para SAP Hybris mediante la API de Flow Service
description: Aprenda a crear una conexión de origen y un flujo de datos para llevar los datos de SAP Hybris al Experience Platform mediante la API de Flow Service.
badge: Beta
hide: true
hidefromtoc: true
...
# Crear una conexión de origen y un flujo de datos para [!DNL SAP Hybris] uso de la API de Flow Service
...
Las secciones siguientes proporcionan información adicional que deberá conocer para conectarse correctamente a [!DNL SAP Hybris] uso del [!DNL Flow Service] API.

### Recopilar credenciales necesarias

Para poder conectarse [!DNL SAP Hybris] para acceder a Experience Platform, debe proporcionar valores para las siguientes propiedades de conexión:

| Credencial | Descripción |
| --- | --- |
| `clientId` | El valor de `clientId` desde la clave de servicio. |
| `clientSecret` | El valor de `clientSecret` desde la clave de servicio. |
| `tokenEndpoint` | El valor de `url` desde la clave de servicio, será similar a `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`. |
| `region` | Su ubicación del centro de datos. La región está presente en el `url` y tiene un valor similar al siguiente `eu10` o `us10`. Por ejemplo, si la variable `url` es `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`, entonces necesitará `eu10`. |

Para obtener más información, consulte la [[!DNL SAP Hybris] documentación](https://help.sap.com/docs/CLOUD_TO_CASH_OD/987aec876092428f88162e438acf80d6/c5fcaf96daff4c7a8520188e4d8a1843.html).

## Connect [!DNL SAP Hybris] a Platform mediante el [!DNL Flow Service] API

A continuación se describen los pasos que debe seguir para autenticar su [!DNL SAP Hybris] origen, crear una conexión de origen y crear un flujo de datos para llevar los datos de cuentas y contactos a Experience Platform.

### Crear una conexión base {#base-connection}

Una conexión base retiene información entre el origen y Platform, incluidas las credenciales de autenticación del origen, el estado actual de la conexión y el ID único de conexión base. El ID de conexión base le permite explorar y navegar por archivos desde el origen e identificar los elementos específicos que desea introducir, incluida la información sobre sus tipos de datos y formatos.

Para crear un ID de conexión base, realice una solicitud de POST al `/connections` extremo al proporcionar su [!DNL SAP Hybris] credenciales de autenticación como parte del cuerpo de la solicitud.
...
La siguiente solicitud crea una conexión base para [!DNL SAP Hybris]:
...
| `auth.params.region` | Su ubicación del centro de datos. La región está presente en el `url` y tiene un valor similar al siguiente `eu10` o `us10`. Por ejemplo, si la variable `url` es `https://subscriptionbilling.authentication.eu10.hana.ondemand.com` usted necesitará `eu10`. |
| `auth.params.clientId` | El valor de `clientId` desde la clave de servicio. |
| `auth.params.clientSecret` | El valor de `clientSecret` desde la clave de servicio. |
| `auth.params.tokenEndpoint` | El valor de `url` desde la clave de servicio, será similar a `https://subscriptionbilling.authentication.eu10.hana.ondemand.com`. |
...
| `{SOURCE_PARAMS}` | Define parámetros para el archivo de origen que desea llevar a Platform. Para recuperar el tipo de formato aceptado para `{SOURCE_PARAMS}`, debe codificar toda la cadena en base64. <br> [!DNL SAP Hybris] admite varias API. Según el tipo de objeto que esté aprovechando, pase uno de los siguientes: <ul><li>`customers`</li><li>`contacts`</li></ul> |

El [!DNL SAP Hybris] La fuente de admite varias API. Según el tipo de objeto que utilice, la solicitud que se va a enviar es la siguiente:
...
Para [!DNL SAP Hybris] La API de cliente tiene el valor para `{SOURCE_PARAMS}` se pasa como `{"object_type":"customers"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` como se muestra a continuación.
...
Para [!DNL SAP Hybris] El valor de la API Contactos es `{SOURCE_PARAMS}` se pasa como `{"object_type":"contacts"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` como se muestra a continuación.
...
La siguiente solicitud crea una conexión de origen para [!DNL SAP Hybris] datos de clientes:
...
| `baseConnectionId` | El ID de conexión base de [!DNL SAP Hybris]. Este ID se generó en un paso anterior. |
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | El formato del [!DNL SAP Hybris] datos que desea introducir. Actualmente, el único formato de datos admitido es `json`. |
| `object_type` | [!DNL SAP Hybris] admite varias API. Para la API de clientes, la variable `object_type` el parámetro debe establecerse en `customers`. |
...
La siguiente solicitud crea una conexión de origen para [!DNL SAP Hybris] datos de contactos:
...
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | El formato del [!DNL SAP Hybris] datos que desea introducir. Actualmente, el único formato de datos admitido es `json`. |
| `object_type` | [!DNL SAP Hybris] admite varias API. Para la API de contactos, la variable `object_type` el parámetro debe establecerse en `contacts`. |
| `path` | Tendrá el mismo valor que seleccione para *`object_type`*. |
...
La siguiente solicitud crea una conexión de destino para [!DNL SAP Hybris]:
...
| `data.format` | El formato del [!DNL SAP Hybris] datos que desea introducir. |
...
La siguiente solicitud crea una asignación para [!DNL SAP Hybris] Datos de API de clientes
...
La siguiente solicitud crea una asignación para [!DNL SAP Hybris] Datos de API de contactos
...
El último paso para obtener datos de [!DNL SAP Hybris] a Platform es crear un flujo de datos. Por ahora, tiene preparados los siguientes valores obligatorios:
