...
# Crear una conexión de origen y un flujo de datos para [!DNL SAP Commerce] uso de la API de Flow Service
...
Las secciones siguientes proporcionan información adicional que deberá conocer para conectarse correctamente a [!DNL SAP Commerce] uso del [!DNL Flow Service] API.
...
## Connect [!DNL SAP Commerce] a Platform mediante el [!DNL Flow Service] API

A continuación se describen los pasos que debe seguir para autenticar su [!DNL SAP Commerce] origen, crear una conexión de origen y crear un flujo de datos para llevar los datos de cuentas y contactos a Experience Platform.

### Cree una conexión base {#base-connection}

Una conexión base retiene información entre el origen y Platform, incluidas las credenciales de autenticación del origen, el estado actual de la conexión y el ID único de conexión base. El ID de conexión base le permite explorar y navegar por archivos desde el origen e identificar los elementos específicos que desea introducir, incluida la información sobre sus tipos de datos y formatos.

Para crear un ID de conexión base, realice una solicitud de POST al `/connections` extremo al proporcionar su [!DNL SAP Commerce] credenciales de autenticación como parte del cuerpo de la solicitud.
...
El [!DNL SAP Commerce] La fuente de admite varias API. Según el tipo de objeto que utilice, la solicitud que se va a enviar es la siguiente:
...
Para [!DNL SAP Commerce] La API de cliente tiene el valor para `{SOURCE_PARAMS}` se pasa como `{"object_type":"customers"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImN1c3RvbWVycyJ9` como se muestra a continuación.
...
Para [!DNL SAP Commerce] El valor de la API Contactos es `{SOURCE_PARAMS}` se pasa como `{"object_type":"contacts"}`. Cuando se codifica en base64, equivale a `eyJvYmplY3RfdHlwZSI6ImNvbnRhY3RzIn0=` como se muestra a continuación.
...
La siguiente solicitud crea una conexión de origen para [!DNL SAP Commerce] datos de clientes:
...
| `baseConnectionId` | El ID de conexión base de [!DNL SAP Commerce]. Este ID se generó en un paso anterior. |
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | El formato del [!DNL SAP Commerce] datos que desea introducir. Actualmente, el único formato de datos admitido es `json`. |
| `object_type` | [!DNL SAP Commerce] admite varias API. Para la API de clientes, la variable `object_type` el parámetro debe establecerse en `customers`. |
...
La siguiente solicitud crea una conexión de origen para [!DNL SAP Commerce] datos de contactos:
...
| `connectionSpec.id` | El ID de especificación de conexión que corresponde a su origen. |
| `data.format` | El formato del [!DNL SAP Commerce] datos que desea introducir. Actualmente, el único formato de datos admitido es `json`. |
| `object_type` | [!DNL SAP Commerce] admite varias API. Para la API de contactos, la variable `object_type` el parámetro debe establecerse en `contacts`. |
| `path` | Tendrá el mismo valor que seleccione para *`object_type`*. |
...
| `data.format` | El formato del [!DNL SAP Commerce] datos que desea introducir. |
...
La siguiente solicitud crea una asignación para [!DNL SAP Commerce] Datos de API de clientes
...
La siguiente solicitud crea una asignación para [!DNL SAP Commerce] Datos de API de contactos
...
El último paso para obtener datos de [!DNL SAP Commerce] a Platform es crear un flujo de datos. Por ahora, tiene preparados los siguientes valores obligatorios:
