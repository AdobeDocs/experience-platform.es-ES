...
Las secciones siguientes proporcionan información adicional que deberá conocer para conectarse correctamente a un sistema CS mediante la [!DNL Flow Service] API.

### Obtención de una conexión base

Para explorar el sistema CS mediante [!DNL Platform] API, debe disponer de un ID de conexión base válido. Si todavía no tiene una conexión de base para el sistema CS con el que desea trabajar, puede crear una mediante los siguientes tutoriales:
...
Una respuesta correcta devuelve una matriz de tablas del sistema CS. Encuentre la tabla que desea traer [!DNL Platform] y tome nota de su `path` propiedad, ya que se le pedirá que la proporcione en el próximo paso para inspeccionar su estructura.
...
Siguiendo este tutorial, ha explorado su sistema CS, ha encontrado la ruta de la tabla en la que desea realizar la ingesta [!DNL Platform]y ha obtenido información sobre su estructura. Puede utilizar esta información en el siguiente tutorial para [recopilar datos de su sistema CS e incluirlos en la plataforma](../collect/customer-success.md).