...
description: Aprenda a utilizar el comando sendMediaEvent para realizar un seguimiento de las sesiones de contenido en Web SDK.
...
El comando `sendMediaEvent` forma parte del componente `streamingMedia` de Web SDK. Puede utilizar este componente para recopilar datos relacionados con las sesiones de contenido en el sitio web. Consulte la `streamingMedia` [documentación](configure/streamingmedia.md) para obtener información sobre cómo configurar este componente.
...
El tipo de evento `media.pauseStart` se usa para hacer un seguimiento de cuándo se pone en pausa una reproducción de contenido. Este evento debe enviarse cuando el usuario presione **[!UICONTROL Pause]**. No hay ningún tipo de evento de reanudación. Se infiere una reanudación cuando se envía un evento `media.play` después de `media.pauseStart`.
...
## Envío de eventos de medios mediante la extensión de etiquetas Web SDK

La extensión de etiquetas Web SDK equivalente a este comando es la acción [**[!UICONTROL Send media event]**](/help/tags/extensions/client/web-sdk/actions/send-media-event.md).
