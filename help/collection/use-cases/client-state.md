...
El propio Edge Network no tiene estado (no mantiene su propia sesión). Sin embargo, hay ciertos casos de uso que requieren persistencia de estado del lado del cliente, como:

* Identificación coherente del dispositivo
* Recopilación y aplicación del consentimiento del usuario
* Mantener ID de sesión de personalización

Edge Network utiliza un protocolo de administración de estado, delegando el aspecto de almacenamiento a su cliente/SDK, e incluye entradas de estado en sus respuestas. En los exploradores, las entradas se almacenan como cookies.
...
Para admitir el etiquetado múltiple (es decir, varias instancias de SDK en la misma propiedad, que podrían hacer referencia a diferentes organizaciones), todas las entradas de estado llevan automáticamente el prefijo `kndctr_` y el ID de organización seguro para la URL.

Cuando el cliente SDK recibe un identificador `state:store` en la respuesta, debe hacer lo siguiente:
...
Al trabajar con clientes del explorador, Edge Network puede mantener automáticamente las entradas como cookies del explorador. Esto permite la compatibilidad con el almacenamiento de estado transparente, ya que los exploradores respetan el protocolo de administración de estado de forma predeterminada.
...
Dado que las entradas siempre están enlazadas a un ámbito específico (dispositivo/aplicación) por su definición, Edge Network solo escribirá el subconjunto compatible con el contexto de solicitud actual. Las entradas no escritas son
...
El indicador `Secure` no está presente en la representación de metadatos de las cookies. Solo se incluye el atributo `SameSite`. En este caso, es responsabilidad del cliente establecer correctamente el indicador `Secure` siempre que el atributo `SameSite` esté presente. Las cookies con `SameSite=None` también deben especificar el atributo `Secure`, ya que requieren un contexto seguro (HTTPS).
