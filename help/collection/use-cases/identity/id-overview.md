...
Adobe Experience Platform Web SDK usa [Adobe Experience Cloud ID (ECID)](/help/identity-service/features/ecid.md) para hacer un seguimiento del comportamiento de los visitantes. Con [!DNL ECIDs], puede asegurarse de que cada dispositivo tenga un identificador único que pueda persistir en varias sesiones y que vincule todas las visitas que se producen durante las sesiones web y entre ellas a un dispositivo específico.
...
* Para los visitantes habituales, Edge Network recupera [!DNL ECID] de la cookie [`kndctr_<orgId>_identity`](https://experienceleague.adobe.com/es/docs/core-services/interface/data-collection/cookies/web-sdk) y lo agrega a la carga de la solicitud.
...
1. Envíe datos directamente al dominio de Edge Network `adobedc.net`. Este método se conoce como [recopilación de datos de terceros](#third-party).
...
La recopilación de datos de terceros implica enviar datos directamente al dominio de Edge Network `adobedc.net`.
...
Use la [preparación de datos para la recopilación de datos](/help/datastreams/data-prep.md) para asignar [!DNL ECID] a un campo de [!DNL XDM]. Esta es la forma recomendada de obtener acceso a [!DNL ECID].
...
![Captura de pantalla de asignación de secuencia de datos](/help/tags/extensions/client/web-sdk/assets/access-ecid-data-prep.png)
...
Para recuperar el CORE ID de un usuario, puede utilizar el comando [`getIdentity()`](/help/collection/js/commands/getidentity.md), como se muestra a continuación.
...
`identityMap` campos se han actualizado mediante el comando `sendEvent`.
...
Cada propiedad de `identityMap` representa identidades que pertenecen a un [área de nombres de identidad](/help/identity-service/features/namespaces.md) en particular. El nombre de propiedad debe ser el símbolo del área de nombres de identidad, que se puede encontrar en la interfaz de usuario de Adobe Experience Platform en &quot;[!UICONTROL Identities]&quot;. El valor de la propiedad debe ser una matriz de identidades pertenecientes a ese área de nombres de identidad.
...
Si actualmente tiene habilitado [reenvío de eventos](/help/tags/ui/event-forwarding/overview.md) y usa `appmeasurement.js` y `visitor.js`, puede mantener habilitada la característica de reenvío de eventos y esto no causará ningún problema. En el back-end, Adobe busca cualquier segmento de AAM y lo añade a la llamada a Analytics. Si la llamada a Analytics contiene esos segmentos, Analytics no llamará a Audience Manager para reenviar ningún dato, por lo que no hay ninguna recopilación de datos doble. Tampoco es necesario utilizar Location Hint al utilizar Web SDK, ya que se llama a los mismos extremos de segmentación en el backend.
