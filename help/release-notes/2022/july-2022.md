...
**Finalizando la compatibilidad con etiquetas en Internet Explorer 10 y 11**

Adobe ya no invierte en la compatibilidad con [Internet Explorer 10 (IE10)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-10-end-of-support) e [Internet Explorer 11 (IE11)](https://docs.microsoft.com/en-us/lifecycle/announcements/internet-explorer-11-end-of-support) para las etiquetas en Adobe Experience Platform. Microsoft ha dejado de ofrecer soporte a estos navegadores, las tecnologías web modernas son cada vez más incompatibles con ellos y el sector en general se ha alejado de ellos. Estos factores hacen que sea difícil y costoso mantener la compatibilidad con versiones anteriores. Aunque Adobe no elimina activamente la compatibilidad con IE10 e IE11, las nuevas funciones de etiquetas ya no se prueban ni optimizan para estos exploradores. Este cambio permite que Adobe ofrezca funciones modernas con mayor rapidez.
...
| Destino | Descripción |
| ----------- | ----------- |
| [Marketo V2](../../destinations/catalog/adobe/marketo-engage.md) | La actualización de destino de Marketo Engage le permite optimizar el proceso de creación de listas estáticas con automatización y permitir a los usuarios introducir campos adicionales en sus posibles clientes. Vea más información sobre las mejoras de Marketo V2 a continuación: <br><ul><li>En el paso **[!UICONTROL Schedule segment]** del flujo de trabajo de activación, en Marketo V1, necesitaba agregar manualmente un **ID de asignación** para exportar correctamente los datos a Marketo. Este paso manual ya no es necesario en Marketo V2.</li><li>En el paso **[!UICONTROL Mapping]** del flujo de trabajo de activación, en Marketo V1, pudo asignar campos XDM solo a tres campos de destino en Marketo: `firstName`, `lastName` y `companyName`. Con la versión 2.0 de Marketo, ahora puede asignar campos XDM a muchos más campos en Marketo. Para obtener más información, lea [atributos admitidos en Marketo V2](../../destinations/catalog/adobe/marketo-engage.md#supported-attributes).  </li></ul> |
...
| --- | --- | --- |
| Clase | [[!UICONTROL Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | Una clase basada en registros que se utiliza para capturar datos meteorológicos. |
| Grupo de campo | [[!UICONTROL Current Weather]](https://github.com/adobe/xdm/blob/master/components/classes/weather.schema.json) | Grupo de campos de las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather] que se utiliza para capturar las condiciones meteorológicas actuales de un código postal. |
| Grupo de campo | [[!UICONTROL Forecasted Weather]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/forecasted-weather.schema.json) | Un grupo de campos para las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather], que se usa para capturar las condiciones meteorológicas previstas para un código postal. |
| Grupo de campo | [[!UICONTROL Product Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/product-triggers.schema.json) | Un grupo de campos para las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather], que se utiliza para capturar déclencheur específicos de productos que aprovechan las condiciones meteorológicas que se sabe que impulsan el comportamiento de los consumidores. |
| Grupo de campo | [[!UICONTROL Relative Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/relative-triggers.schema.json) | Un grupo de campos para las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather], que se utiliza para capturar déclencheur relativos que aprovechan las condiciones meteorológicas que se sabe que impulsan el comportamiento de los consumidores. |
| Grupo de campo | [[!UICONTROL Severe Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/severe-triggers.schema.json) | Un grupo de campos para las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather], que se usa para capturar déclencheur que aprovechan condiciones meteorológicas severas que se sabe que impulsan el comportamiento de los consumidores. |
| Grupo de campo | [[!UICONTROL Weather Triggers]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/shared/weather-triggers.schema.json) | Un grupo de campos para las clases [!UICONTROL XDM ExperienceEvent] y [!UICONTROL Weather], que se utiliza para capturar déclencheur generales que aprovechan las condiciones meteorológicas que se sabe que impulsan el comportamiento de los consumidores. |
| Grupo de campo | [[!UICONTROL Media Interaction Details]](https://github.com/adobe/xdm/blob/master/docs/reference/fieldgroups/experience-event/experienceevent-media.schema.json) | Grupo de campos de la clase [!UICONTROL XDM ExperienceEvent] que captura detalles sobre una interacción de medios. |
| Grupo de campo | [[!UICONTROL MediaAnalytics Interaction Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/experienceevent-media-analytics.schema.json) | Grupo de campos de la clase [!UICONTROL XDM ExperienceEvent] que captura detalles sobre una interacción con los informes de contenidos. |
| Tipo de datos | [[!UICONTROL Advertising details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingdetails.schema.json) | Registra detalles de un recurso publicitario. |
| Tipo de datos | [[!UICONTROL Advertising Pod details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/advertisingpoddetails.schema.json) | Registra detalles sobre un pod de publicidad, que es una secuencia de varios anuncios reproducidos consecutivamente dentro de una sola pausa publicitaria. |
| Tipo de datos | [[!UICONTROL Chapter details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/chapterdetails.schema.json) | Registra detalles de un capítulo o segmento en un fragmento de contenido de vídeo. |
| Tipo de datos | [[!UICONTROL Error details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/errordetails.schema.json) | Registra detalles sobre un error de reproducción de vídeo. |
| Tipo de datos | [[!UICONTROL Player state data information]](https://github.com/adobe/xdm/blob/master/components/datatypes/playerstatedata.schema.json) | Registra detalles relacionados con el estado de un reproductor de vídeo. |
| Tipo de datos | [[!UICONTROL Qoe Data details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/qoedatadetails.schema.json) | Registra detalles de calidad de experiencia (QoE) sobre un evento de reproducción de vídeo. |
| Tipo de datos | [[!UICONTROL Session details information]](https://github.com/adobe/xdm/blob/master/components/datatypes/sessiondetails.schema.json) | Registra los detalles de sesión de un evento de reproducción de vídeo. |
