...
| [Adobe Advertising DSP](../../destinations/catalog/advertising/adobe-advertising-dsp-connection.md) | El destino de Adobe Advertising Cloud [!DNL Demand-Side Platform] (DSP) le permite compartir segmentos de origen autenticados con anunciantes y usuarios aprobados para la activación de campañas con DSP. |
...
| Función | Descripción |
| --- | --- |
| [Configuración de tipo de acceso para flujos de datos](../../datastreams/overview.md#create) | Al crear un nuevo flujo de datos, ahora puede seleccionar qué tipo de solicitudes desea que acepte Edge Network: <ul><li>**[!UICONTROL Mixed Authentication]**: cuando se selecciona esta opción, Edge Network acepta solicitudes autenticadas y no autenticadas. Seleccione esta opción cuando planee usar Web SDK o [Mobile SDK](https://developer.adobe.com/client-sdks/documentation/), junto con la [API de Edge Network](https://developer.adobe.com/data-collection-apis/docs/getting-started/). </li><li>**[!UICONTROL Authenticated Only]**: cuando se selecciona esta opción, Edge Network solo acepta solicitudes autenticadas. Seleccione esta opción cuando planee usar solamente la API de Edge Network y desee evitar que [!DNL Edge Network] procese solicitudes no autenticadas. </li></ul> |
| [Procesar propuestas](/help/collection/js/commands/applypropositions.md) en aplicaciones de una sola página sin incrementar las métricas. | El comando `applyPropositions` recién agregado le permite procesar o ejecutar una matriz de propuestas de [!DNL Target] en aplicaciones de una sola página, sin incrementar las métricas [!DNL Analytics] y [!DNL Target]. Esta mejora aumenta la precisión de los informes. |
| [Uso compartido de ID de móviles a web y entre dominios](/help/collection/use-cases/identity/id-sharing.md) | Adobe Experience Platform Web SDK ahora admite funciones de uso compartido de ID de visitante que le permiten ofrecer experiencias personalizadas de forma más precisa, entre aplicaciones móviles y contenido web móvil, y entre dominios. |
...
| --- | --- | --- |
| Clase | [[!UICONTROL Medication]](https://github.com/adobe/xdm/blob/master/components/classes/medication.schema.json) | Una clase de la industria de la salud que captura detalles acerca de una sustancia utilizada para el tratamiento médico, especialmente un medicamento o un medicamento. |
...
| Clase | [[!UICONTROL Provider]](https://github.com/adobe/xdm/blob/master/components/classes/provider.schema.json) | Una clase del sector sanitario que recopila detalles sobre un proveedor de atención sanitaria. |
| Clase | [[!UICONTROL Payer]](https://github.com/adobe/xdm/blob/master/components/classes/payer.schema.json) | Una clase de la industria de la salud que captura detalles sobre una compañía de seguros. |
| Clase | [[!UICONTROL Live Event Schedule]](https://github.com/adobe/xdm/blob/master/components/classes/live-event-schedule.schema.json) | Una clase de la industria del deporte y del entretenimiento que captura detalles sobre un programa de eventos en vivo, como un horario de conciertos itinerantes o el horario de un equipo deportivo. |
| Clase | [[!UICONTROL Location]](https://github.com/adobe/xdm/blob/master/components/classes/location.schema.json) | Una clase del sector del deporte y el entretenimiento que registra la ubicación de un evento en directo, como una sala de conciertos o un coliseo deportivo. |
| Grupo de campo | [[!UICONTROL Healthcare medication]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/medication/healthcare-medication.schema.json) | Grupo de campos de la clase [!UICONTROL Medication] que captura detalles sobre el medicamento, como el nombre de la marca, el número de lote y la cantidad. |
| Grupo de campo | [[!UICONTROL Healthcare Plan Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/plan/healthcare-plan-details.schema.json) | Grupo de campos de la clase [!UICONTROL Plan] que captura detalles como la red, el tipo y el estado activo. |
| Grupo de campo | [[!UICONTROL Healthcare Provider]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/provider/healthcare-provider-details.schema.json) | Grupo de campo de la clase [!UICONTROL Provider] que captura los detalles de un profesional de la salud individual o de una organización de un centro de salud con licencia para proporcionar servicios de diagnóstico y tratamiento de atención médica. |
| Grupo de campo | [[!UICONTROL Healthcare Member Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/provider/healthcare-provider-details.schema.json) | Grupo de campos de la clase [!UICONTROL XDM Individual Profile] que captura los detalles de una persona que tiene o va a recibir un servicio o atención, como información de contacto, médico de atención primaria e información del plan. |
| Grupo de campo | [[!UICONTROL Sitetool Details]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/industry-verticals/experienceevent-healthcare-sitetool.schema.json) | Grupo de campos de la clase [!UICONTROL XDM ExperienceEvent] que captura información recopilada por herramientas de sitio como bots de chat, encuestas, etc. |
| Grupo de campo | [[!UICONTROL Live Event Ticket Purchase]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/industry-verticals/experienceevent-live-event-ticket-purchase.schema.json) | Grupo de campo de la clase [!UICONTROL XDM ExperienceEvent] que registra el historial de compra de entradas para un evento en directo, como un concierto o un juego deportivo. |
| Grupo de campo | [[!UICONTROL Sports and Entertainment Event Schedule]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/live-event-schedule/sports-entertainment-event-schedule.schema.json) | Grupo de campos de la clase [!UICONTROL Live Event Schedule] que captura más detalles sobre la programación, como el nombre de la atracción, los horarios de apertura de las puertas, etc. |
| Grupo de campo | [[!UICONTROL Sports Entertainment Event Venue]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/location/sports-entertainment-event-venue.schema.json) | Un grupo de campo para la clase [!UICONTROL Location] que captura más detalles acerca del lugar del evento, como la capacidad de asientos y las áreas de mercado designadas (DMA). |
...
| --- | --- | --- |
| Comportamiento | [[!UICONTROL Time-series Schema]](https://github.com/adobe/xdm/blob/master/components/behaviors/time-series.schema.json) | Se ha añadido un tipo de evento de actualización de estados de medios. |
| Grupo de campo | [[!UICONTROL Lodging Reservation]](https://github.com/adobe/xdm/blob/master/components/fieldgroups/experience-event/industry-verticals/experienceevent-lodging-reservation.schema.json) | Se ha agregado una propiedad de pago y alojamiento. |
| Tipo de datos | [[!UICONTROL Media information]](https://github.com/adobe/xdm/blob/master/docs/reference/datatypes/media.schema.json) | Se agregaron los campos state-start y state-end. |
| Extensión | [[!UICONTROL Workfront Change Event]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/workfront/changeevent.schema.json) | Se agregaron dos campos utilizados para almacenar atributos para ayudar a identificar al usuario y la hora de un evento de creación. |
| Extensión | [[!UICONTROL Adobe CJM ExperienceEvent - Message interaction details]](https://github.com/adobe/xdm/blob/master/extensions/adobe/experience/customerJourneyManagement/message-interaction.schema.json) | Se ha añadido información de suscripción, consentimiento, correo electrónico personalizado y datos adicionales en el objeto de página de aterrizaje. |
