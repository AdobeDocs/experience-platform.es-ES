...
| Exploración de datos | Acceso sin problemas a los datos XDM introducidos en [!DNL Platform] integrados con el SDK [!DNL Platform Data Access]. |
...
| Creación de modelos | Plantillas y tiempos de ejecución que permiten a los científicos de datos centrarse en el desarrollo de modelos sin tener que implementar código de infraestructura para acceder a datos y calcular recursos. Puede importar código de modelo y ponerlo en funcionamiento para derivar información de los datos de [!DNL Platform]. |
...
| Puntuación por lotes | Enriquezca [!DNL Real-Time Customer Profile] con perspectivas de aprendizaje automático o escríbalas como conjuntos de datos de nuevo a [!DNL Platform] |
| Programación | Integrado con [!DNL Platform] Orchestration Service para automatizar la formación de modelos, la puntuación y las canalizaciones de características con programaciones definidas por el usuario a través de las API. |
...
| Modelo de objeto de oferta creado previamente | Sin necesidad de generar primero un modelo de datos, un desarrollador de [!DNL Platform] puede aprovechar las relaciones y los esquemas JSON pregenerados para crear un catálogo de ofertas, definir restricciones y reglas de decisión y ensamblar colecciones de ofertas para la toma de decisiones. |
...
* No se puede extender un grupo de campos proporcionado por [!DNL Platform] agregando un campo.
