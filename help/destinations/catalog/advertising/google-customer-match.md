...
Para ayudarle a comprender mejor cómo y cuándo utilizar el destino [!DNL Google Customer Match], aquí hay casos de uso de ejemplo que los clientes [!DNL Adobe Experience Platform] pueden resolver mediante esta característica.
...
>[!NOTE]
>
>Durante el proceso de conexión OAuth2, puede ver &quot;Marketo Test&quot; como el nombre del proyecto OAuth de Google. Este es un comportamiento normal, ya que Adobe utiliza este nombre de proyecto para la integración de Customer Match de Google. Esto no afecta a la configuración de destino.
...
En el paso **[!UICONTROL Segment schedule]**, debe proporcionar [!UICONTROL App ID] al enviar audiencias de [!DNL IDFA] o [!DNL GAID] a [!DNL Google Customer Match].
...
Después de conectarse al destino y establecer un flujo de datos de destino, puede usar la [funcionalidad de supervisión](/help/dataflows/ui/monitor-destinations.md) en [!DNL Real-Time CDP] para obtener información detallada acerca de los registros de perfil activados en el destino en cada ejecución de flujo de datos.
...
Después de completar el flujo de activación, cambie a su cuenta de **[!UICONTROL Google Ads]**. Las audiencias activadas se muestran en su cuenta de Google como listas de clientes. Según el tamaño de la audiencia, algunas audiencias no se rellenan a menos que haya más de 100 usuarios activos para ofrecer.
