...
![Destino de Facebook en la interfaz de usuario de CDP en tiempo real](../../assets/catalog/social/facebook/catalog.png)
...
Un minorista en línea quiere llegar a los clientes existentes a través de las plataformas sociales y mostrarles ofertas personalizadas basadas en sus pedidos anteriores. El minorista en línea puede ingerir direcciones de correo electrónico de su propia CRM a CDP en tiempo real, generar segmentos a partir de sus propios datos sin conexión y enviar estos segmentos a la plataforma [!DNL Facebook] social, optimizando así su inversión en publicidad.
...
Para destinatario en los medios sociales, pueden incorporar los datos del cliente desde su CRM al CDP en tiempo real, utilizando las direcciones de correo electrónico como identificadores.
...
- Su cuenta [!DNL Facebook] de usuario debe tener el **[!DNL Manage campaigns]** permiso habilitado para la cuenta de publicidad que planea usar.
- La cuenta comercial de **Adobe Experience Cloud** debe agregarse como socio publicitario en su [!DNL Facebook Ad Account]. En su lugar, utilice `business ID=206617933627973`. Consulte [Añadir socios a su administrador](https://www.facebook.com/business/help/1717412048538897) comercial en la documentación de Facebook para obtener más información.
   >[!IMPORTANT]
   >
   > When configuring the permissions for Adobe Experience Cloud, you must enable the **Manage campaigns** permission. Es necesario para la integración de [!DNL Real-time CDP].
...
>[!IMPORTANT]
>
>Si decide no usar hash en las direcciones de correo electrónico, CDP en tiempo real lo hará por usted cuando active segmentos en [!DNL Facebook]. En el flujo de trabajo [de](../../ui/activate-destinations.md#activate-data) activación (consulte el paso 5), seleccione la `Email` opción como se muestra a continuación para las direcciones *de correo electrónico* sin procesar y `Email_LC_SHA256` para las direcciones *de correo electrónico* con hash.

![Hashing en activación](../../assets/common/identity-mapping.png)

## Conectar al destino {#connect-destination}

Para conectarse al [!DNL Facebook] destino, consulte Flujo de trabajo [de autenticación de destinos de red](./workflow.md)social.
