...
description: Obtenga información sobre cómo procesar las señales de consentimiento de los clientes en Adobe Experience Platform mediante el estándar Adobe 2.0.
role: Developer
feature: Consent
...
1. Al cargar cada página (o cuando la CMP detecta un cambio en las preferencias de consentimiento), un script personalizado del sitio asigna las preferencias actuales a un objeto XDM estándar. Este objeto se pasa entonces al comando `setConsent` de Experience Platform Web SDK.
1. Cuando se llama a `setConsent`, Experience Platform Web SDK comprueba si los valores de consentimiento son diferentes de los que recibió por última vez. Si los valores son diferentes (o no hay ningún valor anterior), los datos de consentimiento/preferencia estructurados se envían a Adobe Experience Platform.
...
En la versión actual de la compatibilidad con el procesamiento de consentimiento en Experience Platform, Experience Platform Web SDK solo aplica automáticamente el permiso de recopilación de datos (`collect.val`). Aunque se pueden recopilar y mantener consentimientos y preferencias más granulares en los perfiles de los clientes, estas señales adicionales deben aplicarse manualmente en sus propios procesos descendentes.
...
Una vez que tenga los conjuntos de datos y las políticas de combinación para representar los campos de consentimiento requeridos en los perfiles del cliente, el siguiente paso es incorporar los datos de consentimiento en sí a Experience Platform.

Principalmente, debe utilizar Adobe Experience Platform Web SDK para enviar datos de consentimiento a Experience Platform cada vez que CMP detecte eventos de cambio de consentimiento. Si recopila datos de consentimiento en una plataforma móvil, debe utilizar Adobe Experience Platform Mobile SDK. También puede optar por introducir los datos de consentimiento recopilados directamente asignándolos al esquema XDM del conjunto de datos de consentimiento y enviándolos a Experience Platform mediante la ingesta por lotes.

En las subsecciones siguientes se proporcionan detalles sobre cada uno de estos métodos.

### Configuración de Experience Platform Web SDK para procesar datos de consentimiento {#web-sdk}

Una vez configurada la CMP para detectar eventos de cambio de consentimiento en el sitio web, se puede integrar Experience Platform Web SDK para recibir la configuración de consentimiento actualizada y enviarla a Experience Platform en cada carga de página y cada vez que se produzcan eventos de cambio de consentimiento. Consulte la guía sobre [configuración de Web SDK para procesar los datos de consentimiento del cliente](../sdk.md) para obtener más información.

### Configuración de Experience Platform Mobile SDK para procesar datos de consentimiento {#mobile-sdk}

Si se requieren las preferencias de consentimiento del cliente en la aplicación móvil, puede integrar Experience Platform Mobile SDK para recuperar y actualizar la configuración de consentimiento y enviarlas a Experience Platform siempre que se invoque la API de consentimiento.

Consulte la documentación de Mobile SDK para [configurar la extensión móvil de consentimiento](https://developer.adobe.com/client-sdks/documentation/consent-for-edge-network/) y [usar la API de consentimiento](https://developer.adobe.com/client-sdks/documentation/consent-for-edge-network/api-reference/). Para obtener más información sobre cómo gestionar los problemas de privacidad mediante Mobile SDK, consulte la sección [Privacidad y RGPD](https://developer.adobe.com/client-sdks/resources/privacy-and-gdpr/).
...
Para obtener más información sobre las funciones relacionadas con la privacidad de Experience Platform, consulte la descripción general de [gobernanza, privacidad y seguridad en Experience Platform](../../overview.md).
