...
Para programar una consulta, seleccione una plantilla de consulta de la ficha [!UICONTROL Templates] o de la columna [!UICONTROL Template] de la ficha [!UICONTROL Scheduled Queries]. Al seleccionar el nombre de la plantilla, accederá al Editor de consultas.
...
>[!TIP]
>
>Seleccione **[!UICONTROL View schedule]** para navegar al área de trabajo de programaciones y ver si alguna consulta programada se ejecuta de un vistazo.

![Se resaltó el editor de consultas con [!UICONTROL View schedule] y [!UICONTROL Add schedule].](../images/ui/query-schedules/view-add-schedule.png)

Seleccione **[!UICONTROL Add schedule]** para navegar a la [página de detalles de programación](#schedule-details).

También puede seleccionar la ficha **[!UICONTROL Schedules]** debajo del nombre de la consulta.

![Editor de consultas con la ficha Programaciones resaltada.](../images/ui/query-schedules/schedules-tab.png)

Aparecerá el espacio de trabajo programaciones. La interfaz de usuario muestra una lista de las ejecuciones programadas a las que está asociada la plantilla. Seleccione **[!UICONTROL Add Schedule]** para crear una programación.
...
- **[!UICONTROL Hourly]**: la consulta programada se ejecutará cada hora para el período de fecha seleccionado.
- **[!UICONTROL Daily]**: la consulta programada se ejecutará cada X días a la hora y en el período de fecha que haya seleccionado. Tenga en cuenta que la hora seleccionada se encuentra en **UTC**, y no en la zona horaria local.
- **[!UICONTROL Weekly]**: la consulta seleccionada se ejecutará los días de la semana, la hora y el período de fecha que haya seleccionado. Tenga en cuenta que la hora seleccionada se encuentra en **UTC**, y no en la zona horaria local.
- **[!UICONTROL Monthly]**: la consulta seleccionada se ejecutará cada mes en el día, la hora y el período de fecha que haya seleccionado. Tenga en cuenta que la hora seleccionada se encuentra en **UTC**, y no en la zona horaria local.
- **[!UICONTROL Yearly]**: la consulta seleccionada se ejecutará cada año en el día, mes, hora y período de fecha que haya seleccionado. Tenga en cuenta que la hora seleccionada se encuentra en **UTC**, y no en la zona horaria local.
...
Seleccione **[!UICONTROL Create and append into new dataset]** para crear un conjunto de datos cuando ejecute una consulta por primera vez. Las ejecuciones posteriores siguen insertando datos en ese conjunto de datos. Por último, proporcione un nombre y una descripción para el conjunto de datos.
...
![Panel de detalles de la programación con detalles del conjunto de datos y las opciones [!UICONTROL Create and append into new dataset] resaltadas.](../images/ui/query-schedules/dataset-details-create-and-append.png)

Como alternativa, seleccione **[!UICONTROL Append into existing dataset]** seguido del icono del conjunto de datos (![El icono del conjunto de datos.](/help/images/icons/database.png)).
...
Aparecerá el cuadro de diálogo **[!UICONTROL Select output dataset]**.

A continuación, examine los conjuntos de datos existentes o utilice el campo de búsqueda para filtrar las opciones. Seleccione la fila del conjunto de datos que desee utilizar. Los detalles del conjunto de datos se muestran en el panel de la derecha. Seleccione **[!UICONTROL Done]** para confirmar su elección.
...
Al crear una programación, puede inscribir la consulta en la función de cuarentena para salvaguardar los recursos del sistema y evitar posibles interrupciones. La característica de cuarentena identifica y aísla automáticamente las consultas que fallan repetidamente colocándolas en un estado [!UICONTROL Quarantined]. Al poner en cuarentena las consultas después de diez errores consecutivos, puede intervenir, revisar y corregir los problemas antes de permitir más ejecuciones. Esto contribuye a mantener la eficacia operativa y la integridad de los datos.

![El área de trabajo Programaciones de consultas con [!UICONTROL Query Quarantine] resaltado y seleccionado Sí.](../images/ui/query-schedules/quarantine-enroll.png)
...
También puede inscribir una consulta programada en la función de cuarentena desde las acciones en línea de la pestaña [!UICONTROL Scheduled Queries]. Consulte la [documentación de consultas de monitor](./monitor-queries.md#alert-subscription) para obtener más detalles.
...
| `quarantine` | Esta alerta se activa cuando una ejecución de consulta programada se pone en estado de cuarentena. Una vez que una consulta está [inscrita en la característica de cuarentena](#quarantine), cualquier consulta programada que falle diez ejecuciones consecutivas se pone automáticamente en un estado [!UICONTROL Quarantined]. A continuación, una consulta en cuarentena requiere su intervención para que se puedan llevar a cabo más ejecuciones. Nota: Las consultas deben estar inscritas para la función de cuarentena para que pueda suscribirse a las alertas de cuarentena. |
...
>[!NOTE]
>
>Si decide establecer una alerta [!UICONTROL Query Run Delay], debe establecer el tiempo de retraso deseado en minutos en la interfaz de usuario de Experience Platform. Introduzca la duración en minutos. El retraso máximo es de 24 horas (1440 minutos).
...
En la pestaña [!UICONTROL Schedules] de la plantilla, seleccione el ID de programación para ir a la lista de ejecuciones de consulta de la consulta recién programada.
...
Como alternativa, para ver una lista de las ejecuciones programadas de una plantilla de consulta, vaya a la pestaña **[!UICONTROL Scheduled queries]** y seleccione un nombre de plantilla de la lista disponible.
...
|---------------------|----------------------------------|
| [!UICONTROL Query Run ID] | Muestra un identificador único para cada ejecución de consulta, lo que permite realizar un seguimiento y hacer referencia a ejecuciones individuales de consultas programadas. |
| [!UICONTROL Query Run Start] | Indica la fecha y hora de inicio de la ejecución de la consulta para ayudarle a monitorizar cuándo comenzó cada ejecución. |
| [!UICONTROL Query Run Complete] | Muestra la fecha y hora de finalización de la ejecución de la consulta para proporcionar a insight la duración y el estado de ejecución. |
| [!UICONTROL Status] | Muestra el estado actual de la ejecución de la consulta, como `Completed,` `Running,` o `Failed,` para evaluar el resultado rápidamente. |
| [!UICONTROL Datasets] | Enumera los conjuntos de datos utilizados en la ejecución de la consulta para mostrar qué fuentes de datos participaron en la ejecución. |
| [!UICONTROL Compute Hours] | Muestra el tiempo de cálculo utilizado para cada ejecución de consulta, medido en horas. Esto ayuda a realizar un seguimiento del uso de los recursos y optimizar el rendimiento de las consultas. |
...
Puede habilitar, deshabilitar o eliminar una programación del área de trabajo de programaciones de una consulta concreta o del área de trabajo [!UICONTROL Scheduled Queries] que enumera todas las consultas programadas.

Para tener acceso a la ficha [!UICONTROL Schedules] de la consulta elegida, debe seleccionar el nombre de una plantilla de consulta en la ficha [!UICONTROL Templates] o en la ficha [!UICONTROL Scheduled Queries]. Se desplaza al Editor de consultas de esa consulta. En el Editor de consultas, seleccione **[!UICONTROL Schedules]** para acceder al área de trabajo de programaciones.
...
Aparecerá un cuadro de diálogo de confirmación. Seleccione **[!UICONTROL Disable]** para confirmar la acción.
...
Seleccione **[!UICONTROL Delete a schedule]** para eliminar la programación deshabilitada.
...
Como alternativa, la pestaña [!UICONTROL Scheduled Queries] ofrece una colección de acciones en línea para cada consulta programada. Las acciones en línea disponibles incluyen [!UICONTROL Disable schedule] o [!UICONTROL Enable schedule], [!UICONTROL Delete schedule] y [!UICONTROL Subscribe] a las alertas de la consulta programada. Para obtener instrucciones completas sobre cómo eliminar o deshabilitar una consulta programada a través de la ficha Consultas programadas, consulte la [guía de supervisión de consultas programadas](./monitor-queries.md#inline-actions).
