...
En la interfaz de usuario [!DNL Experience Platform], haga clic en **[!UICONTROL Queries]** en el menú de navegación de la izquierda para abrir el espacio de trabajo [!DNL Query Service]. A continuación, haga clic en **[!UICONTROL Create Query]** en la parte superior derecha de la pantalla para comenzar a escribir consultas. Este vínculo está disponible desde cualquiera de las páginas del espacio de trabajo [!DNL Query Service] .
...
Este panel le permite generar un conjunto de datos de salida directamente desde la interfaz de usuario, eliminar o asignar un nombre a la consulta mostrada y ver el código SQL en un formato fácil de copiar en la pestaña **[!UICONTROL SQL Query]**. Este panel también muestra metadatos útiles, como la última vez que se modificó la consulta y quién la modificó, si corresponde. Para generar un conjunto de datos, haga clic en **[!UICONTROL Output Dataset]**. Aparece el cuadro de diálogo **[!UICONTROL Output Dataset]**. Introduzca un nombre y una descripción y, a continuación, haga clic en **[!UICONTROL Run Query]**. El nuevo conjunto de datos se muestra en la pestaña **[!UICONTROL Datasets]** de la interfaz de usuario [!DNL Query Service] en [!DNL Platform].

### Almacenamiento de consultas

[!DNL Query Editor] proporciona una función de guardado que le permite guardar una consulta y trabajar en ella más adelante. Para guardar una consulta, haga clic en **[!UICONTROL Save]** en la esquina superior derecha de [!DNL Query Editor]. Para poder guardar una consulta, debe proporcionarse un nombre para la consulta mediante el panel **[!UICONTROL Query Details]** .
