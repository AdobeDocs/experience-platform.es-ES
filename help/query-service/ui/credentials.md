...
>[!NOTE]
>
>El panel de credenciales no está disponible automáticamente para todos los usuarios. Póngase en contacto con el equipo de su cuenta de Adobe para solicitar que la ficha [!UICONTROL Credentials] se incluya en el área de trabajo del servicio de consultas si la necesita. Si se solicita, este cambio es para toda la organización y está a cargo del equipo de ingeniería de Adobe. No es una configuración controlada por los usuarios.
...
La sección **[!UICONTROL Expiring credentials]** proporciona la siguiente información:

- **[!UICONTROL Host]**: nombre del host al que conectar al cliente. Esto incorpora el nombre de su organización, tal como se ve en la cinta superior de la interfaz de usuario de Experience Platform.
- **[!UICONTROL Port]**: número de puerto del host al que se va a conectar.
- **[!UICONTROL Database]**: nombre de la base de datos a la que conectar un cliente.
- **[!UICONTROL Username]**: el nombre de usuario usado para conectarse al servicio de consultas.
- **[!UICONTROL Password]**: contraseña utilizada para conectarse al servicio de consultas. Las contraseñas de la IU se han cifrado en hash por motivos de seguridad. Seleccione el icono de copia (![El icono de copia.](/help/images/icons/copy.png)) para copiar sus credenciales completas sin hash en el portapapeles.
- **[!UICONTROL PSQL command]**: un comando que ha insertado automáticamente toda la información relevante para conectarse al servicio de consultas mediante PSQL en la línea de comandos.
- **[!UICONTROL Expires]**: fecha y hora de caducidad de las credenciales. La duración de validez predeterminada del token es de 24 horas, pero se puede cambiar en la configuración avanzada de Admin Console.
...
Para acceder a los datos de Customer Journey Analytics en Power BI o Tableau, seleccione el menú desplegable [!UICONTROL Database] y, a continuación, seleccione `prod:cja` de las opciones disponibles. A continuación, copie los parámetros de credenciales de [!DNL Postgres] (host, puerto, base de datos, nombre de usuario y otros) para usarlos en la configuración de Power BI o Tableau.
...
>title="Migrar a la credencial de servidor a servidor de OAuth"
...
Después de estos pasos, se configuran los permisos necesarios en [Adobe Developer Console](https://developer.adobe.com/console/) para que genere las credenciales de servidor a servidor OAuth y use las características de credenciales que caducan o no caducan.

Para obtener información detallada sobre la asignación de permisos, consulte la [documentación de control de acceso](../../access-control/home.md).

### Generar credenciales {#generate-credentials}

Para crear un conjunto de credenciales que no caducan, vuelva a la interfaz de usuario de Experience Platform y seleccione **[!UICONTROL Queries]** en el panel de navegación izquierdo para acceder al área de trabajo [!UICONTROL Queries]. A continuación, seleccione la ficha **[!UICONTROL Credentials]** seguida de **[!UICONTROL Generate credentials]**.
...
- **[!UICONTROL Name]**: nombre de las credenciales que está generando.
- **[!UICONTROL Description]**: (Opcional) una descripción de las credenciales que está generando.
- **[!UICONTROL Assigned to]**: usuario al que se asignarán las credenciales. Este valor debe ser la dirección de correo electrónico del usuario que está creando las credenciales.
- **[!UICONTROL Password]** (opcional) una contraseña opcional para sus credenciales. Si no se ha establecido la contraseña, Adobe la generará automáticamente.
...
>[!IMPORTANT]
>
>Cuando se selecciona **[!UICONTROL Generate credentials]**, se descarga un archivo JSON de configuración en el equipo local. Dado que Adobe **no** registra las credenciales generadas, debe almacenar de forma segura el archivo descargado y mantener un registro de las credenciales.
>
...
Una vez guardadas las credenciales generadas, seleccione **[!UICONTROL Close]**. Ahora puede ver una lista de todas las credenciales que no caducan.
...
### Migrar credenciales a OAuth {#migrate-credentials}

Si utiliza credenciales JWT que no caducan, debe migrar cada una de ellas a OAuth Server-to-Server antes del 30 de junio de 2025 para evitar la interrupción del servicio.

>[!IMPORTANT]
>
>Las credenciales de JWT dejarán de funcionar a partir del 30 de junio de 2025. Debe completar manualmente esta migración para mantener la autorización.

Para aprender a identificar las credenciales afectadas y completar la migración, consulte la [guía de credenciales de servidor a servidor de OAuth para migrar de JWT a servidor](./migrate-jwt-to-oauth.md).

Para preguntas comunes, consulte las [preguntas frecuentes sobre la migración](./migrate-jwt-to-oauth.md#faq).
...
>[!NOTE]
>
>Al conectarse a un host mediante credenciales que no caducan, sigue siendo necesario utilizar todos los parámetros enumerados en la sección [!UICONTROL EXPIRING CREDENTIALS], excepto la contraseña y el nombre de usuario.
...
|---|---|---|
| **Servidor/Host** | El nombre del servidor/host al que se está conectando. <ul><li>Este valor se usa tanto para credenciales que caducan como para credenciales que no caducan y adopta la forma de `server.adobe.io`. El valor se encuentra en **[!UICONTROL Host]** en la sección [!UICONTROL EXPIRING CREDENTIALS].</ul></li> | `acme.platform.adobe.io` |
| **Puerto** | El puerto del servidor/host al que se está conectando. <ul><li>Este valor se usa tanto para credenciales que caducan como para credenciales que no caducan y se encuentra en **[!UICONTROL Port]** en la sección [!UICONTROL EXPIRING CREDENTIALS].</ul></li> | `80` |
| **Database** | La base de datos a la que se está conectando. <ul><li>Este valor se usa tanto para credenciales que caducan como para credenciales que no caducan y se encuentra en **[!UICONTROL Database]** en la sección [!UICONTROL EXPIRING CREDENTIALS]. </ul></li> | `prod:all` |
| **Nombre de usuario** | El nombre de usuario del usuario que se está conectando al cliente externo. <ul><li>Este valor se utiliza tanto para credenciales que caducan como para credenciales que no caducan. Adopta la forma de una cadena alfanumérica antes de `@AdobeOrg`. Este valor se encuentra en **[!UICONTROL Username]**.</li></ul> | `ECBB80245ECFC73E8A095EC9@AdobeOrg` |
| **Contraseña** | La contraseña del usuario que se está conectando al cliente externo. <ul><li>Si usa credenciales que caducan, se encuentra en **[!UICONTROL Password]**, dentro de la sección [!UICONTROL EXPIRING CREDENTIALS].</li><li>Si utiliza credenciales que no caducan, este valor son los argumentos concatenados de technicalAccountID y la credencial tomada del archivo JSON de configuración. El valor de contraseña adopta la forma: `{technicalAccountId}:{credential}`.</li></ul> | <ul><li>Una contraseña de credencial que caduca tiene más de mil caracteres como cadena alfanumérica. No se dará ningún ejemplo.</li><li>Una contraseña de credencial que no caduca es la siguiente:<br>`4F2611B8613DK3670V495N55:3d182fa9e0b54f33a7881305c06203ee`</li></ul> |
