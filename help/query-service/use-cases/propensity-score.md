...
Si aún no lo ha hecho, siga los pasos para [conectar [!DNL Jupyter Notebook] con Adobe Experience Platform Query Service](../clients/jupyter-notebook.md) antes de continuar con esta guía.
...
## Importación de tablas de análisis de Platform en [!DNL Jupyter Notebook] {#import-analytics-tables}

Para generar un modelo de puntuación de tendencia, se debe importar una proyección de los datos de análisis almacenados en Platform en [!DNL Jupyter Notebook]. Desde una [!DNL Python] 3 [!DNL Jupyter Notebook] Conectados al servicio de consultas, los siguientes comandos importan un conjunto de datos de comportamiento del cliente de Luma, una tienda de ropa ficticia. Como los datos de Platform se almacenan con el formato Experience Data Model (XDM), se debe generar un objeto JSON de muestra que se ajuste a la estructura del esquema. Consulte la documentación para obtener instrucciones sobre cómo [generar el objeto JSON de muestra](../../xdm/ui/sample.md).

![El [!DNL Jupyter Notebook] panel con varios comandos resaltados.](../images/use-cases/jupyter-commands.png)

El resultado muestra una vista tabularizada de todas las columnas del conjunto de datos de comportamiento de Luma dentro del [!DNL Jupyter Notebook] panel.

![La salida tabularizada del conjunto de datos de comportamiento del cliente importado de Luma dentro de [!DNL Jupyter Notebook].](../images/use-cases/behavioural-dataset-results.png)

## Preparación de los datos para el aprendizaje automático {#prepare-data-for-machine-learning}

Se debe identificar una columna de destino para entrenar un modelo de aprendizaje automático. Como la tendencia a comprar es el objetivo para este caso de uso, la variable `analytic_action` se elige como columna de destino en los resultados de Luma. El valor `productPurchase` es el indicador de una compra del cliente. El `purchase_value` y `purchase_num` también se eliminan, ya que están directamente relacionadas con la acción de compra del producto.
...
Una técnica llamada *una codificación activa* se utiliza para convertir las variables de datos categóricas para su uso con algoritmos de aprendizaje automático y profundo. Esto, a su vez, mejora las predicciones así como la precisión de clasificación de un modelo. Utilice el `Sklearn` para representar cada valor categórico en una columna independiente.
...
Los datos definidos como `X` se tabulará y aparecerá de la siguiente manera:

![La salida tabularizada de X en [!DNL Jupyter Notebook].](../images/use-cases/x-output-table.png)


Ahora que los datos necesarios para el aprendizaje automático están disponibles, puede ajustar los modelos de aprendizaje automático preconfigurados en [!DNL Python]de `sklearn` biblioteca. [!DNL Logistics Regression] se utiliza para entrenar el modelo de tendencia y permite ver la precisión de los datos de prueba. En este caso, es de aproximadamente el 85 %.

El [!DNL Logistic Regression] y el método de división tren-prueba, utilizado para estimar el rendimiento de los algoritmos de aprendizaje automático, se importan en el siguiente bloque de código:
...
![La visualización de las 10 principales funciones que definen una tendencia a comprar o no comprar.](../images/use-cases/visualized-results.png)
...
- En primer lugar, SQL debe reemplazar a [!DNL Logistics Regression] para obtener la probabilidad de una etiqueta de predicción. El modelo de regresión logística produjo el modelo de regresión `y = wX + c`  donde pesa `w` e interceptar `c` son la salida del modelo. Las funciones SQL se pueden utilizar para multiplicar los pesos y obtener una probabilidad.

- En segundo lugar, el proceso de ingeniería logrado en [!DNL Python] con una codificación hot también debe incorporarse a SQL. Por ejemplo, en la base de datos original, tenemos `geo_county` para almacenar la provincia, pero la columna se convierte en `geo_county=Bexar`, `geo_county=Dallas`, `geo_county=DeKalb`. La siguiente instrucción SQL realiza la misma transformación, donde `w1`, `w2`, y `w3` podrían sustituirse por las ponderaciones aprendidas del modelo en [!DNL Python]:
...
Una vez obtenidos los números, pueden transferirse a una función sigmoidea donde el algoritmo de Regresión Logística produce las predicciones finales. En la declaración siguiente, `intercept` es el número de intersección en la regresión.
        
...
En una situación en la que tiene dos columnas (`c1` y `c2`), si `c1` tiene dos categorías, la [!DNL Logistic Regression] algoritmo entrenado con la siguiente función:
 
...
```
 
El [!DNL Python] el código para automatizar el proceso de traducción es el siguiente:
...
Los resultados tabulados muestran la tendencia a comprar para cada sesión de cliente con `0` lo que significa que no hay tendencia a comprar y `1` lo que significa una tendencia confirmada a comprar.
...
La figura anterior indica que si solo toma 1000 filas para entrenar sus modelos, puede esperar que la precisión caiga entre aproximadamente el 84 % y el 88 %. Puede ajustar la variable `LIMIT` en las consultas del servicio de consultas en función de sus necesidades para garantizar el rendimiento de los modelos.
