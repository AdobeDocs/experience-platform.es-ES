...
La actividad de bots puede afectar a las métricas de análisis y dañar la integridad de los datos. Adobe Experience Platform [!DNL Query Service] se puede usar para mantener la calidad de los datos mediante el proceso de filtrado de bots.
...
## Use [!DNL Query Service] para definir un umbral para la actividad de bots
...
Los umbrales de pico ad hoc establecidos inicialmente no están basados en datos y, por lo tanto, carecen de precisión. Los modelos de aprendizaje automático se pueden utilizar para entrenar parámetros como umbrales. Como resultado, puede aumentar la eficacia de la consulta reduciendo el número de palabras clave `GROUP BY` eliminando las características innecesarias.
...
![Salida estadística del modelo de aprendizaje automático [!DNL Jupyter Notebook].](../images/use-cases/jupiter-notebook-output.png)

Los resultados del modelo que se muestran en el ejemplo anterior son más del 99 % precisos.

Dado que el clasificador del árbol de decisión se puede entrenar con datos de [!DNL Query Service] en una cadencia regular mediante consultas programadas, puede garantizar la integridad de los datos filtrando la actividad de bots con un alto grado de precisión. Mediante el uso de parámetros derivados del modelo de aprendizaje automático, las consultas originales se pueden actualizar con los parámetros de alta precisión creados por el modelo.
...
Al leer este documento, entiende mejor cómo usar [!DNL Query Service] y el aprendizaje automático para determinar y filtrar la actividad de bots.

Otros documentos que muestran los beneficios de [!DNL Query Service] para las perspectivas empresariales estratégicas de su organización son el ejemplo de [caso de uso de exploración abandonado](./abandoned-browse.md).
