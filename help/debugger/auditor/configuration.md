...
| Adobe Advertising: ID de transacción implementado correctamente | 1 | El nombre de propiedad `ev_transid=` no debe estar vacío. | El nombre de propiedad `ev_transid=` no debe quedar sin valor. Si no se introduce un valor, puede causar la pérdida de datos de transacción. Asigne un valor a `ev_transid=` o quite el parámetro del píxel. |
