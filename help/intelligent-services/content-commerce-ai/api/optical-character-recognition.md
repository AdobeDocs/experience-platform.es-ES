...
description: En la API de etiquetado de contenido, el servicio Presencia de texto/Reconocimiento óptico de caracteres (OCR) puede indicar si hay texto en una imagen determinada. Si hay texto, OCR puede devolver el texto.
...
3. is_text_present: 0 o 1 dependiendo de si el texto está presente en la imagen. Si las etiquetas son 0, la lista está vacía.
...
| --- | --- | --- |
| `documents` | Lista de elementos JSON con cada elemento en la lista que representa una imagen. Cualquier parámetro pasado como parte de esta lista, anula el parámetro global especificado fuera de la lista, para el elemento de lista correspondiente. | Sí |
| `sensei:multipart_field_name` | field_name para leer la ruta del archivo de entrada. | Sí |
| `repo:path` | Dirección URL preasignada al recurso de imagen. | Sí |
| `sensei:repoType` | &quot;HTTP&quot; (para presigned-url). | No |
| `dc:format` | Formato codificado de la imagen de entrada. Solo se permiten formatos de imagen como jpeg, jpg, png y tiff para la codificación de imágenes. El formato dc:es compatible con los formatos permitidos. | No |
| `correct_with_dictionary` | ¿Corregir las palabras con un diccionario de inglés? Si no se activa, es posible que se reconozcan palabras que no estén en inglés. El valor predeterminado es True: activado). Tenga en cuenta que cuando el diccionario está activado, no es necesario que siempre aparezca una palabra en inglés. Intentamos corregirlo, pero si no es posible dentro de una cierta distancia de edición, devolvemos la palabra original. | No |
| `filter_with_dictionary` | ¿Filtrar las palabras para que contengan solamente las palabras del diccionario de inglés? Si esta opción está activada, las palabras devueltas siempre pertenecerán al inglés grande , que comprende 470.000 palabras. | No |
| `min_probability` | ¿Cuál es la probabilidad mínima para las palabras reconocidas? El servicio solo devuelve las palabras extraídas de la imagen y que tienen una probabilidad buena que min_probability. El valor predeterminado es 0,2. | No |
| `min_relevance` | ¿Cuál es la relevancia mínima para las palabras reconocidas? El servicio solo devuelve las palabras extraídas de la imagen y que tienen buena relevancia que min_relevant. El valor predeterminado es 0,01. La relevancia se calcula como la fracción del área del cuadro delimitador del texto extraído en comparación con la imagen completa. 0.01 se traduciría en un texto que ocupe al menos el 1% de la imagen. | No |

| Nombre | Tipo de datos | Requerido | Predeterminado | Valores | Descripción |
| -----| --------- | -------- | ------- | ------ | ----------- |
| `repo:path` | string | - | - | - | Dirección url prefirmada de la imagen desde la que se debe extraer el texto. |
| `sensei:repoType` | string | - | - | HTTPS | Tipo de cesión temporal en la que se almacena la imagen. |
| `sensei:multipart_field_name` | string | - | - | - | Utilícelo cuando pase la imagen como un argumento multiparte en lugar de utilizar direcciones url prefirmadas. |
